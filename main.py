...
- Bij een korte mail met alleen naam, woonplaats en medicijn (bijv. "Beemster\nRietveld, Oostzaan\nFenytoine"): de eerste regel is de achternaam, de laatste regel is het medicijn, daartussen staat de straat en/of woonplaats
- Woonplaats staat vaak achter een komma: "Rietveld, Oostzaan" → straat=Rietveld, woonplaats=Oostzaan
  ...
  "notitie": "korte samenvatting van het verzoek"
    ...
    try:
        ...
        if not beste_order:
            ...
            # Probeer woonplaats ook direct uit body te halen als Claude het niet vond
            # Patroon: "straat, Woonplaats" of gewoon "Woonplaats" op een eigen regel
            if not zoek_woonplaats and email_body:
                import re as _re_wp
                for regel in email_body.split("\n"):
                    regel = regel.strip()
                    if "," in regel:
                        # "Rietveld, Oostzaan" → woonplaats is deel na komma
                        delen = [d.strip() for d in regel.split(",")]
                        if len(delen) == 2 and 3 <= len(delen[1]) <= 30:
                            zoek_woonplaats = delen[1].lower()
                            break
            ...
            if zoek_achternaam and len(zoek_achternaam) >= 3:
                ...
                for order in alle_orders:
                    ...
                    # Extra: directe substring match (Beemster in Beemster-Venema)
                    if zoek_achternaam and len(zoek_achternaam) >= 4 and zoek_achternaam in wc_achternaam:
                        naam_score = max(naam_score, 90)
                    if naam_score < 55:
                        continue
