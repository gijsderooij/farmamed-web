        ...
        if _DATABASE_URL:
            try:
                _billing = {
                    ...
                    "first_name": (data.get("patient_naam") or "").split()[0] if (data.get("patient_naam") or "").split() else "",
                    "last_name": " ".join((data.get("patient_naam") or "").split()[1:]),
                ...
                _patient_id = _zoek_of_maak_patient(_billing, _bsn)
                if _patient_id and _gbd:
                    _conn = _pg_conn(); _cur = _conn.cursor()
                    _cur.execute("UPDATE patienten SET geboortedatum=%s, bijgewerkt_op=NOW() WHERE id=%s AND (geboortedatum IS NULL OR geboortedatum='')", (_gbd, _patient_id))
                    _conn.commit(); _cur.close(); _conn.close()
                if _patient_id:
                    _sla_order_op(wc_order_id=str(order["id"]), patient_id=_patient_id, voorschrijver_id=None, medicijn_id=_zoek_medicijn_id(data.get("medicijn","")), hoeveelheid=1, recept_datum=data.get("recept_datum",""))
