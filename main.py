            ...
            try:
                ...
                for part in alle_parts:
                    ...
                    if ct == "text/plain" and "attachment" not in cd:
                        ...
                        # PDF altijd opslaan (ook met CID); afbeeldingen/overige alleen zonder CID
...
async def _identificeer_patient(email="", naam="", telefoon="", bsn="", geboortedatum="", woonplaats=""):
    ...
    if not patient and (email or naam):
        try:
            resp=http_requests.get(f"{wc_url}/wp-json/wc/v3/orders",headers=_wc_auth(wc_key,wc_secret),params={"search":email or naam,"per_page":10,"orderby":"date","order":"desc"},timeout=10)
            if resp.status_code==200:
