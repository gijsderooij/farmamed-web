    ...
    try:
        ...
        for uid in uids:
            ...
            try:
                ...
                body = ""
                bijlagen = []
                def _verwerk_parts(bericht):
                    """Doorzoekt alle delen van een e-mail recursief, ook geneste doorgestuurde mails."""
                    nonlocal body
                    for part in bericht.walk():
                        ct = part.get_content_type()
                        cd = str(part.get("Content-Disposition", ""))
                        cid = part.get("Content-ID", "")

                        # Doorgestuurde mail als bijlage (message/rfc822)
                        if ct == "message/rfc822":
                            geneste = part.get_payload()
                            if isinstance(geneste, list):
                                for genest_bericht in geneste:
                                    _verwerk_parts(genest_bericht)
                            continue
                ...
                # Debug: toon alle MIME parts
                print(f"[Poll] MIME structuur voor {onderwerp[:30]}:")
                for i, part in enumerate(msg.walk()):
                    ct = part.get_content_type()
                    cd = str(part.get("Content-Disposition", ""))[:50]
                    naam = part.get_filename() or ""
                    print(f"[Poll]   part {i}: {ct} | cd={cd} | naam={naam}")
                _verwerk_parts(msg)
