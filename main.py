...
def _init_farmamed_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS medicijnen (
            id SERIAL PRIMARY KEY,
            prk_code TEXT UNIQUE NOT NULL,
            article_code TEXT,
            naam_smarthub TEXT NOT NULL,
            naam_woocommerce TEXT,
            wc_product_id TEXT,
            aangemaakt_op TIMESTAMP DEFAULT NOW()
        )
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS orders (
            id SERIAL PRIMARY KEY,
            wc_order_id TEXT UNIQUE NOT NULL,
            ...
            recept_origineel_naam TEXT,
            recept_opgeslagen_naam TEXT,
            recept_geldig_tot TEXT,
            smarthub_verstuurd INTEGER DEFAULT 0,
            smarthub_verstuurd_op TIMESTAMP,
            smarthub_response TEXT,
            wc_status TEXT,
            wc_datum TEXT,
            wc_totaal TEXT,
            aangemaakt_op TIMESTAMP DEFAULT NOW(),
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS herhaalservice (
            ...
            laatste_herinnering TEXT,
            volgende_herinnering TEXT,
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_orders_geldig_tot ON orders(recept_geldig_tot)")
    ...
    if cur.fetchone()[0] == 0:
        ...
        for m in medicijnen:
            cur.execute(
                "INSERT INTO medicijnen (prk_code, article_code, naam_smarthub, naam_woocommerce) VALUES (%s,%s,%s,%s) ON CONFLICT (prk_code) DO NOTHING",
                m
            )
    conn.commit()
    cur.close()
...
def _zoek_of_maak_patient(billing: dict, bsn: str = "") -> int | None:
    ...
    if not email or email == "info@farmamed.nl":
        return None
    conn = _pg_conn()
    cur = conn.cursor()
    try:
        cur.execute("SELECT id FROM patienten WHERE email = %s", (email,))
        row = cur.fetchone()
        if row:
            if bsn:
                cur.execute(
            ...
            return row[0]
        cur.execute("""
            INSERT INTO patienten (email, bsn, voornaam, achternaam, geboortedatum, adres, postcode, plaats, telefoon)
...
def _zoek_of_maak_voorschrijver(naam: str, agb_code: str = "") -> int | None:
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO voorschrijvers (agb_code, naam) VALUES (%s,%s) ON CONFLICT (agb_code) DO NOTHING RETURNING id",
    ...
    try:
        ...
        for rij in rijen:
            for veld in [rij[1] or "", rij[2] or ""]:
                score = max(
                    fuzz.token_set_ratio(naam_norm, _norm(veld)),
                    fuzz.partial_ratio(naam_norm, _norm(veld)),
                )
...
def _sla_order_op(wc_order_id: str, patient_id, voorschrijver_id, medicijn_id,
    ...
    try:
        ...
        if row:
            ...
            return row[0]
        cur.execute("""
            INSERT INTO orders (wc_order_id, patient_id, voorschrijver_id, medicijn_id, hoeveelheid,
        ...
        """, (wc_order_id, patient_id, voorschrijver_id, medicijn_id, hoeveelheid,
              recept_datum or None, iter_, recept_origineel_naam or None,
              recept_opgeslagen_naam or None, recept_geldig_tot or None))
...
async def download_db(request: Request):
    ...
    return JSONResponse(content={"info": "Database is nu PostgreSQL — gebruik DBeaver om verbinding te maken", "url_hint": "Zie Railway Variables voor DATABASE_URL"})
