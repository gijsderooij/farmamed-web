...
async def _identificeer_patient(
    email: str = "",
    naam: str = "",
    telefoon: str = "",
    bsn: str = "",
    ...
    """
    Centrale patiënt-identificatie functie.
    Zoekt in PostgreSQL én WooCommerce op basis van beschikbare gegevens.
    Geeft terug: gevonden, patient, wc_orders, ontbrekende_velden, match_methode, match_score, suggestie
    """
    from rapidfuzz import fuzz

    _eigen_domeinen = ["farmamed.nl", "apotheekwoerden.nl"]
    def _is_eigen(e):
        return any(str(e or "").lower().strip().endswith("@" + d) for d in _eigen_domeinen)
    ...
    if _DATABASE_URL:
        try:
            ...
            if not patient and naam:
                ...
                for rij in rijen:
                    ...
                    if zoek_ach and zoek_ach in wc_ach:
                        ns = max(ns, 90)
                    if geboortedatum and rij.get("geboortedatum") == geboortedatum and ns >= 60:
                        ns = max(ns, 95)
                    elif woonplaats and rij.get("plaats","").lower() == woonplaats.lower() and ns >= 70:
                        ns = max(ns, 85)
                    if ns > beste_score:
                        beste_score = ns
                        beste_rij = rij
                if beste_rij and beste_score >= 75:
                    patient = beste_rij
                    match_methode = "naam"
                    match_score = beste_score
            ...
            print(f"[Identificeer] DB fout: {e}")

    # === STAP 2: Fallback naar WooCommerce ===
    if not patient and (email or naam):
        try:
            ...
            resp = http_requests.get(
                f"{wc_url}/wp-json/wc/v3/orders",
                headers=_wc_auth(wc_key, wc_secret),
                params={"search": zoek, "per_page": 10, "orderby": "date", "order": "desc"},
                timeout=10,
            )
            if resp.status_code == 200:
                for o in (resp.json() if isinstance(resp.json(), list) else []):
                    ...
                    if _DATABASE_URL and wc_email:
                        try:
                            ...
                            if db_row:
                                db_cols = [d[0] for d in cur_db.description]
                                db_data = dict(zip(db_cols, db_row))
                                patient["id"] = db_data["id"]
                                for veld in ["bsn","geboortedatum","telefoon","adres","postcode","plaats"]:
                                    if db_data.get(veld):
                                        patient[veld] = db_data[veld]
                            cur_db.close()
                            conn_db.close()
                        except Exception as e:
                            print(f"[Identificeer] DB verrijking fout: {e}")
                    break
        except Exception as e:
            print(f"[Identificeer] WC fout: {e}")

    # === STAP 3: Haal WooCommerce orders op ===
    if patient and patient.get("email"):
        try:
            resp = http_requests.get(
                f"{wc_url}/wp-json/wc/v3/orders",
                headers=_wc_auth(wc_key, wc_secret),
                params={"search": patient["email"], "per_page": 10, "orderby": "date", "order": "desc"},
                timeout=10,
            )
            if resp.status_code == 200:
                for o in (resp.json() if isinstance(resp.json(), list) else []):
            ...
            print(f"[Identificeer] WC orders fout: {e}")

    # === STAP 4: Ontbrekende velden en suggestie ===
    verplichte_velden = ["voornaam","achternaam","email","geboortedatum","bsn","adres","postcode","plaats"]
    ontbrekende_velden = [v for v in verplichte_velden if not (patient or {}).get(v)] if patient else verplichte_velden

    laatste_voorschrijver = {}
    for o in wc_orders:
        if o.get("voorschrijver"):
            laatste_voorschrijver = {"naam": o["voorschrijver"], "agb_code": o.get("agb_code","")}
            break

    return {
        "gevonden": bool(patient),
        "patient": patient or {},
        "wc_orders": wc_orders,
        "ontbrekende_velden": ontbrekende_velden,
        "match_methode": match_methode,
        "match_score": match_score,
        "suggestie": "kloon" if patient and wc_orders else "nieuw",
        "laatste_voorschrijver": laatste_voorschrijver,
    }
...
async def identificeer_patient_endpoint(request: Request):
    data = await request.json()
    resultaat = await _identificeer_patient(
