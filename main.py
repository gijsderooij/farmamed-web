                ...
                for part in alle_parts:
                    ...
                    if ct == "text/plain" and "attachment" not in cd:
                        try:
                         ...
                         (ct in ("image/jpeg", "image/png", "image/jpg", "image/gif") and "attachment" in cd and not cid) or \
                         ("attachment" in cd and ct not in ("text/plain", "text/html") and not cid):
...
async def wc_webhook(request: Request):
    """Ontvangt WooCommerce webhooks voor nieuwe/gewijzigde orders."""
    try:
        order = await request.json()
    except Exception:
        return JSONResponse(content={"ok": False, "fout": "Geen JSON"})

    order_id = order.get("id")
    if not order_id:
        return JSONResponse(content={"ok": False, "fout": "Geen order ID"})
    ...
    bsn = (meta.get("billing_bsn") or meta.get("_billing_bsn") or meta.get("bsn") or "").strip()
    geboortedatum = (meta.get("billing_birth") or meta.get("_billing_birth") or "").strip()
    voorschrijver_naam = (meta.get("voorschrijver") or "").strip()
    agb_code = (meta.get("agb_code") or "").strip()
    productnaam = items[0]["name"] if items else ""

    print(f"[Webhook] Order #{order_id} ontvangen — {billing.get('last_name','')} bsn={bool(bsn)}")

    if not _DATABASE_URL:
        return JSONResponse(content={"ok": False, "fout": "Geen database"})

    try:
        # Sla patiënt op
        patient_id = _zoek_of_maak_patient(billing, bsn)

        # Update geboortedatum als aanwezig
        if patient_id and geboortedatum:
            conn_db = _pg_conn()
            cur_db = conn_db.cursor()
            cur_db.execute(
                "UPDATE patienten SET geboortedatum=%s, bijgewerkt_op=NOW() WHERE id=%s AND (geboortedatum IS NULL OR geboortedatum='')",
                (geboortedatum, patient_id)
            )
            # Update BSN als aanwezig
            if bsn:
                cur_db.execute(
        ...
        _sla_order_op(
            ...
            medicijn_id=medicijn_id,
            hoeveelheid=int(items[0].get("quantity", 1)) if items else 1,
            recept_datum=meta.get("recept_datum", ""),
        )

        print(f"[Webhook] Order #{order_id} opgeslagen — patient_id={patient_id}")
        return JSONResponse(content={"ok": True, "order_id": order_id, "patient_id": patient_id})
