...
async def _poll_eenmalig():
    ...
    try:
        ...
        # Alleen verwerkte mails overslaan - niet-verwerkte altijd opnieuw ophalen
        # zodat bijlagen correct worden opgeslagen
        verwerkt_status = {e["uid"]: e for e in bestaande if e.get("verwerkt")}
        # Verwijder niet-verwerkte mails uit cache zodat ze opnieuw worden opgehaald
        niet_verwerkt_uids = {e["uid"] for e in bestaande if not e.get("verwerkt")}
        if niet_verwerkt_uids:
            _verwijder_emails_niet_in(verwerkt_status.keys())
