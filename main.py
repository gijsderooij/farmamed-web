    ...
    try:
        ...
        for status in statussen:
            ...
            while True:
                ...
                _tok = _b64a.b64encode(f"{wc_key}:{wc_secret}".encode()).decode()
                response = http_requests.get(
                    f"{wc_url}/wp-json/wc/v3/orders",
                    params={"status": status, "per_page": 100, "orderby": "date", "order": "desc", "page": pagina},
                    headers={"Accept": "application/json", "Authorization": f"Basic {_tok}", "User-Agent": "curl/7.68.0"},
                    timeout=20,
                )
        ...
        # Sorteer op order ID hoog naar laag (nieuwste bovenaan, ongeacht status)
...
async def markeer_email_verwerkt(request: Request):
    """Markeert een e-mail als verwerkt en verplaatst hem naar Afgehandeld."""
    body = await request.json()
    email_uid = body.get("email_uid", "")
    order_id = body.get("order_id")
    email = _zoek_email_op_uid(email_uid)
