...
def _zoek_article_code(productnaam: str) -> str | None:
    """
    Zoekt de echte Article Code (leidend vanuit Farmasys) op basis van
    productnaam, via dezelfde fuzzy-matchlogica als _zoek_medicijn_id.
    Geeft None terug als er geen code bekend is voor dit medicijn (dan valt
    de aanroepende code terug op de oude, minder betrouwbare naam-afgeleide
    methode) — zo verbetert dit vanzelf naarmate er meer codes gesynchroniseerd zijn.
    """
    from rapidfuzz import fuzz
    if not productnaam or not _DATABASE_URL:
        return None
        ...
        for rij in rijen:
            for veld in [rij[1] or "", rij[2] or ""]:
                ...
                if score > beste_score:
                    beste_score = score
                    beste_code = rij[0]
        return beste_code if beste_score >= 60 else None
    finally:
        cur.close()
        conn.close()
...
def _genereer_edifact(order_data: dict, recept_data: dict = None) -> str:
    ...
    medicijn_code = _zoek_article_code(medicijn) or medicijn.upper().replace(" ", "")[:20]
    try:
...
async def _zoek_patient_kandidaten(email="", naam="", telefoon="", bsn="", geboortedatum="", woonplaats="", medicijn="", max_kandidaten=8, _debug=None):
    ...
    try:
        ...
        if naam:
            ...
            woorden = [w for w in _re.split(r"[\s,]+", naam) if w]
            kandidaat_woorden = []
            for w in woorden:
                ...
                if genorm:
                    kandidaat_woorden.append(genorm)
            if not kandidaat_woorden and naam.split():
                kandidaat_woorden = [_normaliseer_naam(naam.split()[-1])]
            kandidaat_woorden = list(dict.fromkeys(kandidaat_woorden))  # dedupe, volgorde behouden
