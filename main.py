...
{{
  "patient_naam": "volledige naam van de originele patiënt (niet apotheek/arts), of null",
  "patient_email": "e-mailadres van de originele patiënt (niet @farmamed.nl, niet @apotheekwoerden.nl), of null",
  "geboortedatum": "geboortedatum in DD-MM-YYYY formaat. Let op: DDMMJJJJ (bijv. 15031958) omzetten naar 15-03-1958. Of null als niet aanwezig",
  "woonplaats": "woonplaats van de patiënt, of null",
  ...
  "notitie": "korte samenvatting van het verzoek in max 15 woorden"
    ...
    def _is_eigen(email):
        ...
        return any(email.lower().strip().endswith("@" + d) for d in _eigen_domeinen)

    zoek_email = None
    for kandidaat in [email_data.get("patient_email"), afzender_email]:
        if kandidaat and "@" in str(kandidaat) and not _is_eigen(kandidaat):
            zoek_email = kandidaat.strip()
            break

    zoek_naam = email_data.get("patient_naam") or afzender_naam
    zoek_geboortedatum_raw = email_data.get("geboortedatum") or ""
    ...
    try:
        ...
        if not beste_order:
            naam = zoek_naam
            # Normaliseer geboortedatum: DDMMJJJJ of DD-MM-YYYY naar DD-MM-YYYY
            import re as _re_gbd
            zoek_geboortedatum = zoek_geboortedatum_raw or email_data.get("geboortedatum") or ""
            # Converteer DDMMYYYY (8 cijfers zonder streepjes) naar DD-MM-YYYY
            if zoek_geboortedatum and _re_gbd.match(r"^\d{8}$", zoek_geboortedatum.strip()):
                g = zoek_geboortedatum.strip()
                zoek_geboortedatum = f"{g[0:2]}-{g[2:4]}-{g[4:8]}"
            # Extraheer ook geboortedatum direct uit email body (DDMMJJJJ patroon)
            if not zoek_geboortedatum and email_body:
                gbd_match = _re_gbd.search(r"(\d{2})(\d{2})(\d{4})", email_body)
                if gbd_match:
                    d, m_num, y = gbd_match.groups()
                    if 1 <= int(m_num) <= 12 and 1 <= int(d) <= 31 and 1900 <= int(y) <= 2020:
                        zoek_geboortedatum = f"{d}-{m_num}-{y}"
            ...
            if achternaam and len(achternaam) >= 3:
                ...
                resp = http_requests.get(
                    f"{wc_url}/wp-json/wc/v3/orders",
                    headers=_wc_auth(wc_key, wc_secret),
                    params={"search": achternaam, "per_page": 25, "orderby": "date", "order": "desc"},
                    timeout=10,
                )
                orders = resp.json() if resp.status_code == 200 else []
                for order in (orders if isinstance(orders, list) else []):
                    ...
                    # Woonplaats als extra bevestiging
