...
async def get_patient(patient_id: int):
    ...
    try:
        ...
        # Haal ook WooCommerce orders op via email (inclusief orders niet in eigen DB)
        ...
        if email:
            ...
            try:
                ...
                if resp.status_code == 200:
                    bestaande_wc_ids = {str(o.get("wc_order_id")) for o in orders}
                    for o in (resp.json() if isinstance(resp.json(), list) else []):
                        billing = o.get("billing", {})
                        if billing.get("email","").lower() != email.lower(): continue
                        if str(o["id"]) in bestaande_wc_ids: continue
                        meta = {m["key"]:m["value"] for m in o.get("meta_data",[])}
                        items = o.get("line_items", [])
                        orders.append({
                            "id": None,
                            "wc_order_id": str(o["id"]),
                            "wc_status": o.get("status",""),
                            "wc_datum": o.get("date_created","")[:10],
                            "wc_totaal": o.get("total",""),
                            "hoeveelheid": items[0].get("quantity",1) if items else 1,
                            "recept_datum": None,
                            "recept_geldig_tot": None,
                            "smarthub_verstuurd": 0,
                            "smarthub_verstuurd_op": None,
                            "recept_opgeslagen_naam": None,
                            "medicijn": items[0]["name"] if items else "",
                            "voorschrijver": meta.get("voorschrijver",""),
                            "agb_code": meta.get("agb_code",""),
                        })
                    orders.sort(key=lambda o: o.get("wc_datum") or "", reverse=True)
            except Exception as e:
                print(f"[patient] WC orders fout: {e}")
