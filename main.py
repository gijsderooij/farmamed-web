...
@app.get("/api/debug-order-meta")
async def debug_order_meta(order_id: int):
    """
    TIJDELIJK — alleen om de exacte meta_data-sleutel van de 'herhaalrecept'-
    checkbox op de WooCommerce-afrekenpagina te achterhalen. Haalt de VOLLEDIGE,
    ongefilterde order rechtstreeks bij de WooCommerce-API op (dus ook
    underscore-velden die het 'Aangepaste velden'-blok in wp-admin verbergt).
    Verwijderen zodra de sleutel bekend is — geen productiefunctie.
    """
    ...
    try:
        ...
        return JSONResponse(content={
            ...
            "line_items_meta": [
                {"naam": it.get("name"), "meta_data": it.get("meta_data", [])}
