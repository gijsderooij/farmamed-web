...
async def wc_webhook(request: Request):
    ...
    productnaam = items[0]["name"] if items else ""
    print(f"[Webhook] Order #{order_id} — {billing.get('last_name','')} bsn={bool(bsn)} gbd={bool(geboortedatum)}")
    print(f"[Webhook] Meta keys: {list(meta.keys())[:10]}")
    if not _DATABASE_URL:
    ...
    try:
        ...
        print(f"[Webhook] OK — patient_id={patient_id}")
        return JSONResponse(content={"ok": True})
    except Exception as e:
