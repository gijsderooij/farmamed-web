        ...
        if email:
            try:
                ...
                if resp_o.status_code == 200:
                    for o in (resp_o.json() if isinstance(resp_o.json(), list) else []):
                        billing = o.get("billing", {})
                        if billing.get("email", "").lower() != email.lower():
                            continue
                        meta = {m["key"]: m["value"] for m in o.get("meta_data", [])}
...
async def _zoek_patient_kandidaten(email="", naam="", telefoon="", bsn="", geboortedatum="", woonplaats="", medicijn="", max_kandidaten=8):
    """
    ...
    try:
        ...
        if email:
            cur.execute(f"SELECT {kolommen} FROM patienten WHERE LOWER(email)=%s", (email,))
            cols = [d[0] for d in cur.description]
            for r in cur.fetchall():
        ...
        if naam:
            ...
            if fragment:
                cur.execute(
                    ...
                    "AND achternaam ILIKE %s ORDER BY achternaam, voornaam LIMIT 500",
                    (f"%{fragment}%",)
                )
                cols = [d[0] for d in cur.description]
                for r in cur.fetchall():
...
async def identificeer_patient_kandidaten_endpoint(request: Request):
    ...
    return JSONResponse(content={"kandidaten": kandidaten})
...
BELANGRIJK: e-mails naar recept@farmamed.nl bevatten vaak onderaan een
e-mailhandtekening van de apotheekmedewerker zelf (naam, telefoonnummer,
e-mailadres, functietitel). Dit is NOOIT de patiënt — negeer deze
handtekeninginformatie volledig, ook als het de meest recente of enige
naam/contactgegevens in de mail lijkt te zijn. Extraheer alleen gegevens
die daadwerkelijk over de patiënt/het verzoek zelf gaan.
