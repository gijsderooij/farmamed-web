    ...
    try:
        ...
        for uid in uids:
            ...
            try:
                ...
                if "bereidingsafdeling" in onderwerp or "tav" in onderwerp.lower():
                    print(f"[Poll] DEBUG {onderwerp[:40]}: {len(alle_parts)} parts")
                    for _i, _p in enumerate(alle_parts):
                        print(f"[Poll]   {_i}: {_p.get_content_type()} | {str(_p.get('Content-Disposition',''))[:60]} | {_p.get_filename() or ''}")
                ...
                for part in alle_parts:
                    ...
                    if ct == "text/plain" and "attachment" not in cd:
                        try:
                         ...
                         (ct in ("image/jpeg", "image/png", "image/jpg", "image/gif") and ("attachment" in " ".join(cd.split()))) or \
                         ("attachment" in " ".join(cd.split()) and ct not in ("text/plain", "text/html")):
                        if cid:
                            continue
