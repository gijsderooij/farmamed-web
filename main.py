...
async def verwerk_mt940(bestand: UploadFile = File(...), request: Request = None):
    """MT940 parser: betaling-gecentreerd, 0-350 euro, verbeterde ordernummer-detectie."""
    import re as _re
    from rapidfuzz import fuzz
    ...
    def _extraheer_ordernr(remi_tekst):
        if not remi_tekst:
            return ""
        rc = remi_tekst.replace(FARMAMED_AGB, "")
        # Weert datums
        rc = _re.sub(r'\b\d{1,2}[-/]\d{1,2}[-/]\d{2,4}\b', '', rc)
        rc = _re.sub(r'\b\d{8}\b', '', rc)
        rc = _re.sub(r'\b20\d{2}\b', '', rc)
        rc_nospace = _re.sub(r'\s+', '', rc)
        # Expliciete vermelding
        nm = _re.search(
            r'(?:ordernummer|ordernr|order|fact(?:uur)?|bestelling)[nr\.#:\s]*(\d{3,5})',
            rc_nospace, _re.IGNORECASE
        )
        if nm:
            k = nm.group(1)
            if 3 <= len(k) <= 5 and not k.startswith('020'):
                return k
        # Los 4-cijferig getal
        for m in _re.finditer(r'\b(\d{4})\b', rc):
            c = m.group(1)
            if c.startswith('0') or c.startswith('20') or c.startswith('19'):
                continue
            if c in (FARMAMED_AGB[:4], FARMAMED_AGB[4:]):
                continue
            return c
        return ""
    ...
    for blok in _re.split(r":61:", tekst)[1:]:
        try:
            ...
            remi = ""
            rm = _re.search(r"/REMI/(.+?)(?=/EREF/|/CSID/|/ORDP/|$)", oms_raw, _re.DOTALL)
            if rm:
                remi = _re.sub(r"\s+", " ", rm.group(1)).strip()
            order_nr = _extraheer_ordernr(remi)
            betalingen.append({"datum": datum, "bedrag": bedrag, "naam": naam,
                               "remi": remi[:100], "order_nr": order_nr})
        except Exception:
            continue

    if not betalingen:
        return JSONResponse(content={"fout": "Geen betalingen gevonden (€0-350) in MT940 bestand"})
    ...
    for b in betalingen:
        match = None
        # Stap 1: ordernummer
        if b["order_nr"] and b["order_nr"] in order_index:
            o = order_index[b["order_nr"]]
            bedrag_klopt = abs(b["bedrag"] - float(o["totaal"])) < 0.10
            match = {"order": o, "methode": "ordernummer", "bedrag_klopt": bedrag_klopt, "score": 100}
            gebruikt.add(b["order_nr"])
        # Stap 2: naam fuzzy + bedrag
        if not match and b["naam"]:
            ...
            for oid, o in order_index.items():
                ...
                bedrag_klopt = abs(b["bedrag"] - float(o["totaal"])) < 0.10
                if score >= 65 and bedrag_klopt and score > beste_score:
                    beste_score = score
                    beste_order = o
            if beste_order:
                match = {"order": beste_order, "methode": "naam+bedrag",
                         "bedrag_klopt": True, "score": beste_score}
                gebruikt.add(str(beste_order["id"]))
        resultaten.append({"betaling": b, "match": match})
