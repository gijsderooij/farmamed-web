        ...
        for uid in uids:
            ...
            try:
                ...
                for part in alle_parts:
                    ...
                    # Normaliseer CD: verwijder newlines, tabs en extra spaties
                    cd_raw = part.get("Content-Disposition") or ""
                    if hasattr(cd_raw, '__str__'):
                        cd = " ".join(str(cd_raw).replace("\r","").replace("\n","").replace("\t"," ").split())
                    else:
                        cd = ""
