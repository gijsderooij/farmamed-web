...
def _zoek_of_maak_patient(billing: dict, bsn: str = "") -> int | None:
    ...
    conn = _farmamed_db()
    try:
        row = conn.execute("SELECT id FROM patienten WHERE email = ?", (email,)).fetchone()
        if row:
...
def _zoek_of_maak_voorschrijver(naam: str, agb_code: str = "") -> int | None:
    """Zoekt bestaande voorschrijver op AGB-code of maakt nieuwe aan."""
    if not naam and not agb_code:
        return None
    conn = _farmaced_db() if False else _farmamed_db()
    try:
        if agb_code:
            row = conn.execute("SELECT id FROM voorschrijvers WHERE agb_code = ?", (agb_code,)).fetchone()
            if row:
                return row["id"]
        cursor = conn.execute(
            "INSERT OR IGNORE INTO voorschrijvers (agb_code, naam) VALUES (?,?)",
            (agb_code or None, naam or "Onbekende voorschrijver")
        )
        if cursor.lastrowid:
            conn.commit()
            return cursor.lastrowid
        row = conn.execute("SELECT id FROM voorschrijvers WHERE naam = ?", (naam,)).fetchone()
...
def _zoek_medicijn_id(productnaam: str) -> int | None:
    """Zoekt medicijn in eigen DB via fuzzy matching op naam."""
    from rapidfuzz import fuzz
    conn = _farmamed_db()
    try:
        rijen = conn.execute("SELECT id, naam_smarthub, naam_woocommerce FROM medicijnen").fetchall()
        beste_score = 0
        beste_id = None
        naam_lower = productnaam.lower()
        for rij in rijen:
            for veld in [rij["naam_smarthub"] or "", rij["naam_woocommerce"] or ""]:
                score = max(
                    fuzz.token_set_ratio(naam_lower, veld.lower()),
                    fuzz.partial_ratio(naam_lower, veld.lower()),
                )
...
def _sla_order_op(wc_order_id: str, patient_id, voorschrijver_id, medicijn_id,
    ...
    """Slaat order op in eigen DB of werkt bij. Geeft order_id terug."""
    conn = _farmamed_db()
    try:
        row = conn.execute("SELECT id FROM orders WHERE wc_order_id = ?", (wc_order_id,)).fetchone()
        if row:
            conn.execute("""UPDATE orders SET
                patient_id=COALESCE(?,patient_id),
                voorschrijver_id=COALESCE(?,voorschrijver_id),
        ...
        cursor = conn.execute("""INSERT INTO orders
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (wc_order_id, patient_id, voorschrijver_id, medicijn_id, hoeveelheid,
             recept_datum, iter_, recept_origineel_naam, recept_opgeslagen_naam, recept_geldig_tot))
        conn.commit()
        return cursor.lastrowid
    finally:
        conn.close()
            ...
            try:
                ...
                for part in alle_parts:
                    ...
                    if ct == "text/plain" and "attachment" not in cd:
                        ...
                        print(f"[Poll] Bijlage kandidaat: {ct} | cid={bool(cid)} | cd={cd[:40]}")
                        print(f"[Poll] Bijlage kandidaat: {ct} cid={bool(cid)} cd={cd[:50]}")
                        if cid:
                            continue
...
async def _identificeer_patient(email="", naam="", telefoon="", bsn="", geboortedatum="", woonplaats=""):
    """Centrale patiënt-identificatie. Zoekt in PostgreSQL én WooCommerce."""
    from rapidfuzz import fuzz
    ...
    if _DATABASE_URL:
        try:
            ...
            if bsn and len(bsn)==9:
                ...
                row=cur.fetchone()
                if row: patient=dict(zip([d[0] for d in cur.description],row)); match_methode="bsn"; match_score=100
            if not patient and email:
                ...
                row=cur.fetchone()
                if row: patient=dict(zip([d[0] for d in cur.description],row)); match_methode="email"; match_score=100
            if not patient and naam:
                ...
                for rij in rijen:
                    ...
                    if geboortedatum and rij.get("geboortedatum")==geboortedatum and ns>=55: ns=max(ns,95)
                ...
                if beste_rij and beste_score>=65: patient=beste_rij; match_methode="naam"; match_score=beste_score
            cur.close(); conn.close()
        except Exception as e: print(f"[Identificeer] DB fout: {e}")

    # Stap 2: Zoek in WooCommerce als niet gevonden in DB
    if not patient and (email or naam):
        try:
            zoek=email or naam
            resp=http_requests.get(f"{wc_url}/wp-json/wc/v3/orders",headers=_wc_auth(wc_key,wc_secret),params={"search":zoek,"per_page":20,"orderby":"date","order":"desc"},timeout=10)
            if resp.status_code==200:
                kandidaten=[]
                for o in (resp.json() if isinstance(resp.json(),list) else []):
                    billing=o.get("billing",{}); wc_email=billing.get("email","").lower()
                    meta={m["key"]:m["value"] for m in o.get("meta_data",[])}
                    if email and wc_email==email:
                        wc_gbd=meta.get("billing_birth") or meta.get("_billing_birth") or ""
                        patient={"id":None,"email":wc_email,"voornaam":billing.get("first_name",""),"achternaam":billing.get("last_name",""),"geboortedatum":wc_gbd,"bsn":meta.get("billing_bsn") or meta.get("_billing_bsn") or meta.get("bsn") or "","telefoon":billing.get("phone",""),"adres":billing.get("address_1",""),"postcode":billing.get("postcode",""),"plaats":billing.get("city",""),"wc_customer_id":str(o.get("customer_id",""))}
                        match_methode="email_wc"; match_score=100; break
                    elif naam:
                        wc_naam=f"{billing.get('first_name','')} {billing.get('last_name','')}".strip()
                        score=max(fuzz.token_sort_ratio(_normaliseer_naam(naam),_normaliseer_naam(wc_naam)),fuzz.token_set_ratio(_normaliseer_naam(naam),_normaliseer_naam(wc_naam)))
                        wc_gbd=meta.get("billing_birth") or meta.get("_billing_birth") or ""
                        if geboortedatum and wc_gbd and geboortedatum==wc_gbd and score>=55: score=max(score,90)
                        if score>=65:
                            kandidaten.append({"score":score,"id":None,"email":wc_email,"voornaam":billing.get("first_name",""),"achternaam":billing.get("last_name",""),"geboortedatum":wc_gbd,"bsn":meta.get("billing_bsn") or meta.get("_billing_bsn") or meta.get("bsn") or "","telefoon":billing.get("phone",""),"adres":billing.get("address_1",""),"postcode":billing.get("postcode",""),"plaats":billing.get("city",""),"wc_customer_id":str(o.get("customer_id","")),"wc_order_id":o["id"]})
                if not patient and kandidaten:
                    kandidaten.sort(key=lambda k:k["score"],reverse=True)
                    patient=kandidaten[0]; match_methode="naam_wc"; match_score=patient["score"]
                # Verrijk beste match met PostgreSQL data
                if patient and not patient.get("id") and _DATABASE_URL and patient.get("email"):
                    try:
                        conn_db=_pg_conn(); cur_db=conn_db.cursor()
                        cur_db.execute("SELECT id,bsn,geboortedatum,telefoon,adres,postcode,plaats FROM patienten WHERE LOWER(email)=%s",(patient["email"],))
                        db_row=cur_db.fetchone()
                        if db_row:
                            db_data=dict(zip([d[0] for d in cur_db.description],db_row))
    ...
    # Stap 3: Haal WooCommerce orders op voor de gevonden patiënt
    ...
    # Stap 4: Bepaal ontbrekende velden en suggestie
