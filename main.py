    ...
    if count == 0:
        conn.executemany(
            ...
            [
                ...
                ("90725277","FENYTOINE 10% SAW CREME 30G","Fenytoïne crème 10% (SAW-crème)"),
                ...
                for part in alle_parts:
                    ...
                    if ct == "text/plain" and "attachment" not in cd:
                        try:
                            ...
                            pass
                    elif ct in ("application/pdf", "image/jpeg", "image/png", "image/jpg", "image/gif") or                          ("attachment" in cd and ct not in ("text/plain", "text/html")):
                        if cid:
                            continue
...
SMARTHUB_MEDICIJNEN = {
    "amitriptyline 5% in dimethylsulfoxide 50% saw":  {"code": "90008757", "name": "AMITRIPTYLINE 5% IN DIMETHYLSULFOXIDE 50% SAW"},
    "amitriptyline 10% in dimethylsulfoxide 50% saw": {"code": "90049254", "name": "AMITRIPTYLINE 10% IN DIMETHYLSULFOXIDE 50% SAW 30G"},
    "amitriptyline 10% saw":                          {"code": "90563289", "name": "AMITRIPTYLINE 10% SAW CREME 10%"},
    "amitriptyline 5% saw":                           {"code": "90522792", "name": "AMITRIPTYLINE 5% SAW CREME 30g"},
    "amitriptilyne 5% in dsmo 50% saw":               {"code": "90276591", "name": "AMITRIPTILYNE 5% IN DSMO 50% SAW"},
    "baclofen 5% fenytoine 5% saw":                   {"code": "90089751", "name": "BACLOFEN 5%/ FENYTOINE 5% SAW CREME 30G"},
    "baclofen 5% saw":                                {"code": "90644283", "name": "BACLOFEN 5% SAW CREME 30G"},
    "clonidine 0,1% saw":                             {"code": "90846768", "name": "CLONIDINE 0,1% SAW CREME 30G"},
    "clonidine 0,2% saw":                             {"code": "90887265", "name": "CLONIDINE 0,2% SAW CREME 30G"},
    "fenytoine 5% saw":                               {"code": "90684780", "name": "FENYTOINE 5% SAW CREME 30G"},
    "fenytoine 10% saw":                              {"code": "90725277", "name": "FENYTOINE 10% SAW CREME 30G"},
    "fenytoine 20% saw":                              {"code": "90765774", "name": "FENYTOINE 20% SAW CREME 30G"},
    "gabapentine 10% saw":                            {"code": "90927762", "name": "GABAPENTINE 10% SAW CREME 30G"},
    "ketamine 10% saw":                               {"code": "90603786", "name": "KETAMINE 10% SAW CREME 30G"},
    "ketamine 10% in saw":                            {"code": "90236094", "name": "KETAMINE 10% IN SAW CREME"},
    "tretinoine 0,02% saw":                           {"code": "90058461", "name": "TRETINOINE 0,02% SAW CREME"},
    "tretinoine 0,05% saw":                           {"code": "90098958", "name": "TRETINOINE 0,05% SAW CREME"},
    "tretinoine 0,1% saw":                            {"code": "90139455", "name": "TRETINOINE 0,1% SAW CREME"},
    "proefbehandeling kopsky":                        {"code": "90170745", "name": "PROEFBEHANDELING PIJNSTILLENDE CREMES SAW KOPSKY"},
    "proefbehandeling v02":                           {"code": "90017964", "name": "PROEFBEHANDELING PIJNSTILLENDE CREMES SAW V02"},
}
...
def _zoek_smarthub_artikel(productnaam: str) -> dict:
    ...
    for sleutel, artikel in SMARTHUB_MEDICIJNEN.items():
        if sleutel in naam_lower or naam_lower in sleutel:
            return artikel

    # Fuzzy match als fallback
    beste_score = 0
    beste_artikel = None
    for sleutel, artikel in SMARTHUB_MEDICIJNEN.items():
        score = fuzz.token_set_ratio(naam_lower, sleutel)
        if score > beste_score:
            beste_score = score
