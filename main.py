...
def _zoek_of_maak_patient(billing: dict, bsn: str = "") -> int | None:
    """Zoekt patiënt op email of maakt nieuwe aan. Geeft patient_id terug."""
    email = (billing.get("email") or "").lower().strip()
    if not email or email in ("info@farmamed.nl", "info@apotheekwoerden.nl", "onbekend@farmamed.nl"):
        return None
    ...
    try:
        ...
        if row:
            ...
            return row[0]
        cur.execute("""
            INSERT INTO patienten (email, bsn, voornaam, achternaam,
        ...
        print(f"[DB] _zoek_of_maak_patient fout: {e}")
        return None
    finally:
        cur.close()
        conn.close()
...
def _zoek_of_maak_voorschrijver(naam: str, agb_code: str = "") -> int | None:
    """Zoekt voorschrijver op AGB of naam, maakt aan als niet gevonden."""
    ...
    try:
        if agb_code:
            ...
            if row:
                return row[0]
        naam = (naam or "Onbekende voorschrijver").strip()
        cur.execute("""
            INSERT INTO voorschrijvers (agb_code, naam)
            VALUES (%s, %s)
            ON CONFLICT (agb_code) DO UPDATE SET naam=EXCLUDED.naam
            RETURNING id
        ...
        print(f"[DB] _zoek_of_maak_voorschrijver fout: {e}")
        return None
    finally:
        cur.close()
        conn.close()
...
def _zoek_medicijn_id(productnaam: str) -> int | None:
    """Zoekt medicijn_id via fuzzy matching op productnaam."""
    ...
    try:
        ...
        print(f"[DB] _zoek_medicijn_id fout: {e}")
        return None
    finally:
        cur.close()
        conn.close()
...
def _sla_order_op(wc_order_id: str, patient_id, voorschrijver_id, medicijn_id,
    ...
    """Slaat order op in PostgreSQL of werkt apotheek-velden bij. Geeft order_id terug."""
    if not _DATABASE_URL or not wc_order_id:
        return 0
    conn = _pg_conn()
    cur = conn.cursor()
    try:
        ...
        print(f"[DB] _sla_order_op fout: {e}")
        return 0
    finally:
        cur.close()
        conn.close()
