...
def _zoek_patient_via_telefoon(telefoon: str) -> dict | None:
    """Zoekt een patiënt op via telefoonnummer (genormaliseerd, land-/nul-onafhankelijk)."""
    if not telefoon or not _DATABASE_URL:
        return None
    telefoon_norm = _normaliseer_telefoon(telefoon)
    if not telefoon_norm:
        return None
    conn = _pg_conn()
    cur = conn.cursor()
    try:
        cur.execute("SELECT id, voornaam, achternaam, telefoon FROM patienten WHERE telefoon IS NOT NULL AND telefoon != ''")
        for pid, voornaam, achternaam, tel in cur.fetchall():
            if _normaliseer_telefoon(tel) == telefoon_norm:
                return {"id": pid, "voornaam": voornaam, "achternaam": achternaam}
        return None
    finally:
        cur.close()
        conn.close()
...
def _whatsapp_auth_headers() -> dict:
    token = os.getenv("WHATSAPP_TOKEN", "")
    return {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}


def _whatsapp_verstuur_tekst(telefoon: str, tekst: str) -> bool:
    """Verstuurt een gewoon tekstbericht via de WhatsApp Cloud API."""
    phone_number_id = os.getenv("WHATSAPP_PHONE_NUMBER_ID", "")
    if not phone_number_id or not os.getenv("WHATSAPP_TOKEN", ""):
        return False
    try:
        resp = http_requests.post(
            ...
            json={"messaging_product": "whatsapp", "recipient_type": "individual",
                  "to": telefoon, "type": "text", "text": {"body": tekst}},
            timeout=10,
        )
...
def _whatsapp_verstuur_lijst(telefoon: str, tekst: str, opties: list[tuple[str, str]]) -> bool:
    """
    Verstuurt een keuzemenu (interactive list message) — max 3 knoppen zou
    hier niet volstaan (wij hebben 4 opties), dus een lijst i.p.v. losse
    knoppen. opties = [(id, titel), ...], max 10.
    """
    phone_number_id = os.getenv("WHATSAPP_PHONE_NUMBER_ID", "")
    if not phone_number_id or not os.getenv("WHATSAPP_TOKEN", ""):
        return False
    rows = [{"id": oid, "title": titel[:24]} for oid, titel in opties[:10]]
    try:
        resp = http_requests.post(
            f"https://graph.facebook.com/v23.0/{phone_number_id}/messages",
            headers=_whatsapp_auth_headers(),
            json={
                "messaging_product": "whatsapp", "recipient_type": "individual", "to": telefoon,
                "type": "interactive",
                "interactive": {
                    "type": "list",
                    "body": {"text": tekst},
                    "action": {"button": "Kies een optie", "sections": [{"title": "Opties", "rows": rows}]},
                },
            },
            timeout=10,
        )
...
def _detecteer_bestandstype(inhoud: bytes) -> str:
    """
    Herkent het WERKELIJKE bestandstype aan de eerste bytes ('magic number'),
    in plaats van te vertrouwen op een content-type-header die een externe
    bron (bijv. de WordPress-plugin) kan meesturen — die bleek soms altijd
    'application/pdf' te claimen, ook als de inhoud feitelijk een JPEG is.
    """
    if not inhoud:
    ...
    if inhoud.startswith(b"\x89PNG\r\n\x1a\n"):
        return "image/png"
    if inhoud.startswith(b"GIF87a") or inhoud.startswith(b"GIF89a"):
        return "image/gif"
    return "application/octet-stream"


def _afbeelding_naar_pdf(inhoud: bytes, afbeelding_type: str) -> bytes:
    """
    Zet een afbeelding (JPEG/PNG/GIF) om naar een echte, geldige PDF — zodat
    een recept dat als foto is binnengekomen alsnog consistent als PDF
    gedownload/geopend kan worden, in plaats van alleen de bestandsnaam
    'eerlijker' te maken (.jpg i.p.v. .pdf, wat prima werkt maar niet was
    wat gewenst is: altijd een PDF, ongeacht de herkomst).
    """
    import fitz
    doc = fitz.open()
    afbeelding_doc = fitz.open(stream=inhoud, filetype=afbeelding_type.split("/")[-1])
    pagina_afbeelding = afbeelding_doc[0]
    breedte, hoogte = pagina_afbeelding.rect.width, pagina_afbeelding.rect.height
    pagina = doc.new_page(width=breedte, height=hoogte)
    pagina.insert_image(pagina.rect, stream=inhoud)
    pdf_bytes = doc.tobytes()
    doc.close()
    afbeelding_doc.close()
    return pdf_bytes
...
@app.get("/api/debug-vergelijk-orders")
async def debug_vergelijk_orders(order_ids: str):
    """
    TIJDELIJK — vergelijkt voor de opgegeven ordernummers (komma-gescheiden)
    de ruwe WooCommerce-data met onze eigen orders/patienten-tabellen, om te
    zien waar BSN/geboortedatum precies uit elkaar lopen. Verwijderen zodra
    de oorzaak duidelijk is — geen productiefunctie.
    """
    ...
    resultaten = []
    for order_id in [o.strip() for o in order_ids.split(",") if o.strip()]:
        ...
        try:
            ...
            item["woocommerce"] = {
                ...
                "alle_bsn_achtige_velden": {k: v for k, v in meta.items() if "bsn" in k.lower()},
                "alle_geboortedatum_achtige_velden": {k: v for k, v in meta.items() if "birth" in k.lower() or "geboorte" in k.lower()},
            }
        except Exception as e:
            item["woocommerce"] = {"fout": str(e)}

        if _DATABASE_URL:
            conn = _pg_conn(); cur = conn.cursor()
            try:
                cur.execute("SELECT id, patient_id, voorschrijver_id FROM orders WHERE wc_order_id = %s", (str(order_id),))
                orderrij = cur.fetchone()
                if orderrij:
                    item["eigen_orders_tabel"] = {"order_pk": orderrij[0], "patient_id": orderrij[1], "voorschrijver_id": orderrij[2]}
                    if orderrij[1]:
                        cur.execute(
                            "SELECT id, voornaam, achternaam, geboortedatum, bsn, email, telefoon, bijgewerkt_op FROM patienten WHERE id = %s",
                            (orderrij[1],),
                        )
                        patrij = cur.fetchone()
                        if patrij:
                            item["eigen_patienten_tabel"] = {
                                "id": patrij[0], "naam": f"{patrij[1]} {patrij[2]}".strip(),
                                "geboortedatum": str(patrij[3]) if patrij[3] else "",
                                "bsn": patrij[4], "email": patrij[5], "telefoon": patrij[6],
                                "bijgewerkt_op": str(patrij[7]) if patrij[7] else "",
                            }
                else:
                    item["eigen_orders_tabel"] = None
            finally:
...
@app.get("/gedeeld.js")
async def gedeeld_js():
    # Gedeelde front-end logica (kandidatenzoek, vergelijkingstabel,
    # order-aanmaak-flow) voor emails.html en straks ook Balie (index.html).
    # Staat in de hoofdmap, naast main.py — vandaar geen "templates/" hier.
    js_path = BASE_DIR / "gedeeld.js"
...
async def startup_event():
    if _DATABASE_URL:
        try:
            ...
            # WhatsApp-verzoeken: eenvoudige taken die het apotheekteam moet
            # oppakken (géén automatische order/adreswijziging/factuur-actie).
            _cur.execute("""
                CREATE TABLE IF NOT EXISTS whatsapp_verzoeken (
                    id SERIAL PRIMARY KEY,
                    telefoon TEXT NOT NULL,
                    patient_id INTEGER,
                    status TEXT NOT NULL DEFAULT 'wacht_op_keuze',
                    keuze TEXT,
                    bericht_tekst TEXT,
...
def _verplaats_email_imap(uid_str: str) -> bool:
    ...
    if not all([imap_server, imap_user, imap_pass]):
        return False
    try:
        conn = _imaplib.IMAP4_SSL(imap_server, imap_port, timeout=15)
        conn.login(imap_user, imap_pass)
        conn.select("INBOX")
        try:
            status, sel_data = conn.select(afgehandeld_map)
...
def _woorden_voor_match(s: str) -> list:
    """
    Zelfde normalisatie/opsplitsing als _woordenVoorMatch in gedeeld.js —
    zodat medicijn-matching in de backend en de frontend-dropdown consistent
    zijn (en dezelfde bug-klasse hier niet opnieuw kan ontstaan).
    """
    import re
    import unicodedata as _ud
    genorm = _ud.normalize('NFD', (s or '').lower())
    genorm = ''.join(c for c in genorm if _ud.category(c) != 'Mn')
    genorm = re.sub(r'(\d+)\s*mg\s*/\s*g\b', lambda m: f"{int(m.group(1)) / 10:g}%", genorm)
    return [w for w in re.split(r'[^a-z0-9]+', genorm) if len(w) >= 3 or w.isdigit()]


def _eerste_woord_komt_overeen(tekst: str, kandidaat: str) -> bool:
    """
    Het eerste woord (bij Nederlandse apotheeknamen vrijwel altijd de
    werkzame stof) moet overeenkomen — voorkomt dat twee totaal verschillende
    medicijnen (bijv. Baclofen en Ketamine) alsnog een hoge fuzzy-score
    krijgen puur omdat ze generieke woorden delen ("SAW", "crème", een
    percentage). Zelfde beveiliging als _besteNaamMatch in gedeeld.js.
    """
    woorden_tekst = _woorden_voor_match(tekst)
    woorden_kandidaat = _woorden_voor_match(kandidaat)
    if not woorden_tekst or not woorden_kandidaat:
        return False
    a, b = woorden_tekst[0], woorden_kandidaat[0]
    return a == b or a.startswith(b) or b.startswith(a)


# Directe koppeling tussen elke MEDICIJN_OPTIES-dropdown-waarde (gedeeld.js,
# exact dezelfde strings — het eerste element van elke rij daar) en de
# bijbehorende SmartHUB-artikelcode. Voor een keuze uit onze eigen,
# gecontroleerde lijst is fuzzy matching niet nodig: we WETEN precies welk
# artikel bedoeld wordt, dus geen risico op een verkeerd artikel (zoals
# Baclofen dat op Ketamine matchte) puur door gedeelde generieke woorden.
MEDICIJN_OPTIES_ARTIKELCODES = {
    "Amitriptyline crème 5% (SAW-crème)": {"code": "90495794", "name": "AMITRIPTYLINE 5% SAW CREME 30g"},
    "Amitriptyline crème 5% in DMSO 50% (SAW-crème)": {"code": "90981758", "name": "AMITRIPTYLINE 5% IN DIMETHYLSULFOXIDE 50% SAW"},
    "Amitriptyline crème 10% (SAW-crème)": {"code": "90536291", "name": "AMITRIPTYLINE 10% SAW CREME 10%"},
    "Amitriptyline crème 10% in DMSO 50% (SAW-crème)": {"code": "90022256", "name": "AMITRIPTYLINE 10% IN DIMETHYLSULFOXIDE 50% SAW 30G"},
    "Baclofen crème 5% (SAW-crème)": {"code": "90617285", "name": "BACLOFEN 5% SAW CREME 30G"},
    "Baclofen 5% / Fenytoïne 5% (SAW-crème)": {"code": "90062753", "name": "BACLOFEN 5%/ FENYTOINE 5% SAW CREME 30G"},
    "Clonidine crème 0,1% (SAW-crème)": {"code": "90819770", "name": "CLONIDINE 0,1% SAW CREME 30G"},
    "Clonidine crème 0,2% (SAW-crème)": {"code": "90860267", "name": "CLONIDINE 0,2% SAW CREME 30G"},
    "Fenytoïne crème 5% (SAW-crème)": {"code": "90657782", "name": "FENYTOINE 5% SAW CREME 30G"},
    "Fenytoïne crème 10% (SAW-crème)": {"code": "90698279", "name": "FENYTOINE 10% SAW CREME 30G"},
    "Fenytoïne crème 20% (SAW-crème)": {"code": "90738776", "name": "FENYTOINE 20% SAW CREME 30G"},
    "Gabapentine crème 10% (SAW-crème)": {"code": "90900764", "name": "GABAPENTINE 10% SAW CREME 30G"},
    "Ketamine crème 10% (SAW-crème)": {"code": "90576788", "name": "KETAMINE 10% SAW CREME 30G"},
    "Ketamine crème 10% in SAW (SAW-crème)": {"code": "90209096", "name": "KETAMINE 10% IN SAW CREME"},
    "Tretinoïne crème 0.02% FNA - tegen huidveroudering (30 gram)": {"code": "90031463", "name": "TRETINOINE 0,02% SAW CREME"},
    "Tretinoïne crème 0.05% FNA - tegen huidveroudering (30 gram)": {"code": "90071960", "name": "TRETINOINE 0,05% SAW CREME"},
    "Tretinoïne crème 0.1% FNA - tegen huidveroudering (30 gram)": {"code": "90112457", "name": "TRETINOINE 0,1% SAW CREME"},
    "Proefbehandeling pijnstillende crèmes SAW Kopsky": {"code": "90143747", "name": "PROEFBEHANDELING PIJNSTILLENDE CREMES SAW KOPSKY"},
    "Proefbehandeling pijnstillende crèmes SAW V02": {"code": "90990965", "name": "PROEFBEHANDELING PIJNSTILLENDE CREMES SAW V02"},
    # 'Isoso in Lidocaine', 'Naltrexon (Low Dose)...', 'Tadalafil 5mg...' en
    # 'Oxybutynine' staan bewust NIET in deze lijst — daar is (nog) geen
    # bekende SmartHUB-artikelcode voor; die vallen terug op de
    # database-opzoeking/fuzzy-match/'ONBEKEND' hieronder, in plaats van dat
    # hier een gok wordt vastgelegd.
}
...
def _zoek_smarthub_artikel(productnaam: str) -> dict:
    ...
    Volgorde: 1) exacte koppeling met een MEDICIJN_OPTIES-dropdownwaarde
    (MEDICIJN_OPTIES_ARTIKELCODES — geen giswerk, want de medewerker koos dit
    letterlijk uit onze eigen, gecontroleerde lijst), 2) database
    (medicijnen.article_code — hier komen zowel eigen SAW-crèmes als
    'gewone' Z-index-artikelen zoals Tadalafil binnen via
    sync_artikelcodes.py), 3) de hardgecodeerde SMARTHUB_MEDICIJNEN-lijst
    (legacy, dekt alleen de eigen crèmes), 4) 'ONBEKEND' als niets matcht.
    Stappen 2-3 gebruiken fuzzy matching (met de eis dat de werkzame stof/
    eerste woord overeenkomt) — alleen relevant voor productnamen die niet
    letterlijk uit de dropdown komen, bijv. nog niet-gereconcilieerde
    OCR-tekst.
    """
    if productnaam in MEDICIJN_OPTIES_ARTIKELCODES:
        return MEDICIJN_OPTIES_ARTIKELCODES[productnaam]
    ...
    # Exacte substring match eerst — ook hier het eerste woord vereisen,
    # anders kan een kort, generiek deel van de naam (bijv. "5% saw") al
    # genoeg zijn voor een verkeerde substring-match.
    for sleutel, artikel in SMARTHUB_MEDICIJNEN.items():
        sleutel_norm = _norm(sleutel)
        if (sleutel_norm in naam_norm or naam_norm in sleutel_norm) and _eerste_woord_komt_overeen(productnaam, sleutel):
            return artikel

    # Fuzzy match met genormaliseerde namen — alleen kandidaten waarvan het
    # eerste woord (de werkzame stof) overeenkomt.
    beste_score = 0
    beste_artikel = None
    for sleutel, artikel in SMARTHUB_MEDICIJNEN.items():
        if not _eerste_woord_komt_overeen(productnaam, sleutel):
            continue
        sleutel_norm = _norm(sleutel)
        score = max(
    ...
    try:
        ...
        for rij in rijen:
            for veld in [rij[1] or "", rij[2] or ""]:
                if not veld or not _eerste_woord_komt_overeen(productnaam, veld):
                    continue
                score = max(
                    fuzz.token_set_ratio(naam_norm, _normaliseer_naam(veld)),
                    fuzz.partial_ratio(naam_norm, _normaliseer_naam(veld)),
...
@app.get("/api/zoek-artikel")
async def zoek_artikel(naam: str = ""):
    """Zoekt de SmartHUB-artikelcode + -naam op voor een medicijnnaam — dezelfde
    matching (medicijnen.article_code, dan SMARTHUB_MEDICIJNEN, dan 'ONBEKEND')
    als /api/verstuur-smarthub daadwerkelijk gebruikt bij het versturen."""
    if not naam:
        return JSONResponse(content={"code": "", "name": ""})
    return JSONResponse(content=_zoek_smarthub_artikel(naam))


async def _bouw_en_bewaar_smarthub_bericht(order_id, client_bericht=None, client_patient_id=None):
    """
    Bouwt het SmartHUB-verstrekkingsverzoek-bericht en slaat de reconciliatie
    (patiënt, voorschrijver, medicijn, order) op in onze eigen database.

    Gedeeld door 'Goedkeuren' (/api/order/{id}/bevestig-verwerking — alleen
    de lokale opslag, geen extern bericht) en 'SmartHUB versturen'
    (/api/verstuur-smarthub — dezelfde opslag, plus het bericht ook echt naar
    SmartHUB sturen). Beide gebruiken dus precies dezelfde brondata en
    dezelfde opslaglogica; geen kans meer dat de twee uit elkaar lopen.

    Idempotent: _zoek_of_maak_patient/_zoek_of_maak_voorschrijver/
    _sla_order_op zijn allemaal find-or-create/upsert, dus meerdere keren
    aanroepen voor dezelfde order (bijv. eerst bij Goedkeuren, later nog een
    keer bij SmartHUB versturen) overschrijft niets verkeerd — het werkt de
    bestaande rij bij in plaats van 'm te dupliceren.

    Retourneert (smarthub_bericht, opslag_fout). opslag_fout is None bij
    succes, anders een leesbare foutmelding — wordt nu ECHT teruggegeven aan
    de aanroeper, niet meer alleen naar de server-log geprint.
    """
    ...
    if resp.status_code != 200:
        return None, f"Order niet gevonden: HTTP {resp.status_code}"
    ...
    if client_bericht:
        smarthub_bericht = client_bericht
        c = smarthub_bericht.get("client", {})
        item0 = (smarthub_bericht.get("items") or [{}])[0]
        voorschrijver = item0.get("prescription", {}).get("prescriber", {})
        artikel = item0.get("article", {})
        geboortedatum = c.get("dateOfBirth", "")
        bsn = c.get("socialSecurityNumber", "")
        voorschrijver_naam = voorschrijver.get("name") or "Onbekende voorschrijver"
        voorschrijver_agb = voorschrijver.get("code") or ""
        productnaam = artikel.get("name", "")
        hoeveelheid = item0.get("quantity", {}).get("value") or 1
        recept_datum = item0.get("prescription", {}).get("date") or datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # Voor _zoek_of_maak_patient (alleen gebruikt als client_patient_id
        # ontbreekt): naam/email uit het bevestigde bericht, niet uit de
        # mogelijk-nog-onjuiste ruwe WooCommerce-billing.
        contacten = c.get("contacts", [])
        billing = {
            **billing,
            "first_name": c.get("firstname", billing.get("first_name", "")),
            "last_name": c.get("surname", billing.get("last_name", "")),
            "email": next((ct["value"] for ct in contacten if ct.get("type") == "Email"), billing.get("email", "")),
            "phone": next((ct["value"] for ct in contacten if ct.get("type") == "Phone"), billing.get("phone", "")),
        }
    else:
    ...
    # organizationCode is server-config, geen keuze van de medewerker — altijd
    # hier zetten (ook als de client al een bericht meestuurde), zodat een
    # afwijkende SMARTHUB_ORG-instelling nooit per ongeluk genegeerd wordt.
    smarthub_bericht["organizationCode"] = SMARTHUB_ORG

    # Sla op in eigen Farmamed database
    try:
        patient_id = client_patient_id or _zoek_of_maak_patient(billing, bsn, geboortedatum)
        ...
        return smarthub_bericht, None
    except Exception as e:
        print(f"[SmartHUB] DB fout: {e}")
        return smarthub_bericht, str(e)


@app.post("/api/order/{order_id}/bevestig-verwerking")
async def bevestig_verwerking(order_id: int, request: Request):
    """
    Hoort bij de 'Goedkeuren'-knop in Orders: slaat de door de medewerker
    bevestigde reconciliatie (patiënt, voorschrijver, medicijn) DIRECT op in
    onze eigen database — onafhankelijk van of er ooit op '📡 SmartHUB'
    gedrukt wordt. Voorheen gebeurde deze opslag alleen als bijeffect van het
    versturen naar SmartHUB, waardoor 'Goedkeuren + recept downloaden'
    zonder ook op SmartHUB te drukken niets opsloeg — terwijl dat wél als
    afgerond aanvoelde.
    """
    body = await request.json()
    client_bericht = body.get("bericht")
    client_patient_id = body.get("patient_id")

    _, opslag_fout = await _bouw_en_bewaar_smarthub_bericht(order_id, client_bericht, client_patient_id)
    if opslag_fout:
        return JSONResponse(content={"ok": False, "fout": opslag_fout})
    return JSONResponse(content={"ok": True})
...
async def verstuur_smarthub(request: Request):
    """
    Verstuurt een verstrekkingsverzoek naar SmartHUB.

    Als de client een 'bericht' meestuurt (het exacte JSON-object dat al als
    preview is getoond — opgebouwd uit de door de medewerker bevestigde
    velden in de vergelijkingstabel, incl. eventuele correcties), wordt dát
    ONGEWIJZIGD verstuurd. Zonder 'bericht' (backwards compatible, oudere
    aanroepen) valt deze route terug op de oude aanpak: alles zelf afleiden
    uit de ruwe WooCommerce-order.

    De lokale opslag (patiënt/voorschrijver/order) gebeurt normaliter al bij
    'Goedkeuren' (zie /api/order/{id}/bevestig-verwerking hierboven) — deze
    route doet het hier nog een keer (idempotent, dus onschadelijk) voor het
    geval SmartHUB verstuurd wordt zonder dat Goedkeuren is langsgekomen.
    """
    body = await request.json()
    ...
    smarthub_bericht, opslag_fout = await _bouw_en_bewaar_smarthub_bericht(order_id, client_bericht, client_patient_id)
    if smarthub_bericht is None:
        return JSONResponse(content={"ok": False, "fout": opslag_fout})
    if opslag_fout:
        print(f"[SmartHUB] Lokale opslag mislukt, bericht wordt wél verstuurd: {opslag_fout}")
    ...
    try:
        ...
        return JSONResponse(content={
            ...
            "opslag_fout": opslag_fout,
        })
    except Exception as e:
        return JSONResponse(content={"ok": False, "fout": str(e), "bericht": smarthub_bericht, "opslag_fout": opslag_fout})
...
async def wc_webhook(request: Request):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"[Webhook] #{order_id} — aanvullende orderdata ophalen mislukt (HTTP {resp.status_code}), ga door met de oorspronkelijke webhook-payload (mogelijk onvolledig)")
    except Exception as e:
        print(f"[Webhook] #{order_id} — aanvullende orderdata ophalen mislukt ({e}), ga door met de oorspronkelijke webhook-payload (mogelijk onvolledig)")
    billing = order.get("billing", {})
    meta = {m["key"]: m["value"] for m in order.get("meta_data", [])}
    items = order.get("line_items", [])
    bsn = (meta.get("billing_bsn") or meta.get("_billing_bsn") or meta.get("bsn") or "").strip()
...
async def whatsapp_webhook_verify(request: Request):
    """
    Meta's verplichte verificatie-handshake bij het instellen van de webhook:
    stuurt hub.verify_token mee, wij bevestigen door hub.challenge terug te
    geven als het token overeenkomt met onze eigen (geheime) instelling.
    """
    params = request.query_params
    verify_token = os.getenv("WHATSAPP_VERIFY_TOKEN", "")
    if params.get("hub.mode") == "subscribe" and params.get("hub.verify_token") == verify_token and verify_token:
        return PlainTextResponse(content=params.get("hub.challenge", ""))
    return JSONResponse(status_code=403, content={"fout": "Verificatie mislukt"})


# Menu-opties: (id, titel-in-lijst, weergavetekst-in-bevestiging)
_WHATSAPP_MENU = [
    ("herhaalbestelling", "Herhaalbestelling", "een herhaalbestelling"),
    ("factuur", "Factuur ontvangen", "een factuur"),
    ("adreswijziging", "Adreswijziging", "een adreswijziging"),
    ("anders", "Iets anders", "iets anders"),
]


def _whatsapp_stuur_menu(telefoon: str, naam: str = ""):
    aanhef = f"Hallo {naam}, " if naam else "Hallo, "
    tekst = f"{aanhef}waarmee kunnen we u helpen? Kies hieronder een optie."
    opties = [(oid, titel) for oid, titel, _ in _WHATSAPP_MENU]
    _whatsapp_verstuur_lijst(telefoon, tekst, opties)


@app.post("/api/whatsapp-webhook")
async def whatsapp_webhook(request: Request):
    """
    Verwerkt binnenkomende WhatsApp-berichten. Herkent het nummer, toont een
    keuzemenu (max 4 vaste, taakgerichte opties — geen open AI-gesprek, dat
    is sinds 2026 sowieso niet meer toegestaan), en legt de keuze + eventuele
    vervolgtekst vast als taak voor het apotheekteam. Verandert NOOIT
    automatisch een adres, verstuurt geen factuur en maakt geen order aan —
    dat blijft een bewuste, menselijke handeling.
    """
    if not _DATABASE_URL:
        return JSONResponse(content={"ok": False, "fout": "Geen database verbinding"})

    try:
        body = await request.json()
        entry = (body.get("entry") or [{}])[0]
        change = (entry.get("changes") or [{}])[0]
        waarde = change.get("value", {})
        berichten = waarde.get("messages", [])
        if not berichten:
            return JSONResponse(content={"ok": True})  # statusupdate (afgeleverd/gelezen), geen bericht

        bericht = berichten[0]
        telefoon = bericht.get("from", "")
        if not telefoon:
            return JSONResponse(content={"ok": True})

        # Tekst uit een gewoon bericht, of de gekozen list-item-id uit een menukeuze
        if bericht.get("type") == "interactive":
            gekozen_id = (bericht.get("interactive", {}).get("list_reply") or {}).get("id", "")
            tekst_in = gekozen_id
        else:
            tekst_in = (bericht.get("text", {}).get("body") or "").strip()

        conn = _pg_conn(); cur = conn.cursor()
        try:
            cur.execute(
                "SELECT id, status FROM whatsapp_verzoeken WHERE telefoon=%s AND status != 'afgerond' "
                "ORDER BY aangemaakt_op DESC LIMIT 1",
                (telefoon,)
            )
            open_verzoek = cur.fetchone()

            if open_verzoek and open_verzoek[1] == "wacht_op_adres":
                # Dit bericht IS het nieuwe adres — vastleggen als taak, niet automatisch verwerken
                verzoek_id = open_verzoek[0]
                cur.execute(
                    "UPDATE whatsapp_verzoeken SET status='afgerond', bericht_tekst=%s, bijgewerkt_op=NOW() WHERE id=%s",
                    (tekst_in, verzoek_id)
                )
                conn.commit()
                _whatsapp_verstuur_tekst(telefoon, "Dank u, we hebben uw adreswijziging genoteerd. Ons team verwerkt dit voor u.")
                return JSONResponse(content={"ok": True, "afgehandeld": "adreswijziging"})

            if open_verzoek and open_verzoek[1] == "wacht_op_keuze":
                verzoek_id = open_verzoek[0]
                gekozen = next((m for m in _WHATSAPP_MENU if m[0] == tekst_in), None)
                if not gekozen:
                    # Onherkenbare invoer — menu opnieuw sturen i.p.v. giswerk
                    _whatsapp_verstuur_tekst(telefoon, "Sorry, dat begrepen we niet. Kies een van de opties hieronder.")
                    patient = _zoek_patient_via_telefoon(telefoon)
                    _whatsapp_stuur_menu(telefoon, patient["voornaam"] if patient else "")
                    return JSONResponse(content={"ok": True, "actie": "menu_herhaald"})

                oid, _titel, weergave = gekozen
                if oid == "adreswijziging":
                    cur.execute(
                        "UPDATE whatsapp_verzoeken SET status='wacht_op_adres', keuze=%s, bijgewerkt_op=NOW() WHERE id=%s",
                        (oid, verzoek_id)
                    )
                    conn.commit()
                    _whatsapp_verstuur_tekst(telefoon, "Wat is uw nieuwe adres? (straat, huisnummer, postcode en woonplaats)")
                    return JSONResponse(content={"ok": True, "actie": "adres_gevraagd"})
                else:
                    cur.execute(
                        "UPDATE whatsapp_verzoeken SET status='afgerond', keuze=%s, bijgewerkt_op=NOW() WHERE id=%s",
                        (oid, verzoek_id)
                    )
                    conn.commit()
                    _whatsapp_verstuur_tekst(telefoon, f"Dank u, we hebben uw verzoek voor {weergave} genoteerd. Ons team neemt dit voor u op.")
                    return JSONResponse(content={"ok": True, "afgehandeld": oid})

            # Geen open verzoek: nieuw gesprek — patiënt herkennen en menu tonen
            patient = _zoek_patient_via_telefoon(telefoon)
            cur.execute(
                "INSERT INTO whatsapp_verzoeken (telefoon, patient_id, status) VALUES (%s, %s, 'wacht_op_keuze')",
                (telefoon, patient["id"] if patient else None)
            )
            conn.commit()
            _whatsapp_stuur_menu(telefoon, patient["voornaam"] if patient else "")
            return JSONResponse(content={"ok": True, "actie": "menu_gestuurd", "patient_herkend": patient is not None})
        finally:
            cur.close(); conn.close()
    except Exception as e:
        print(f"[WhatsApp webhook] Fout: {e}")
        return JSONResponse(content={"ok": False, "fout": str(e)})
...
async def analyseer_recept(bestand: UploadFile = File(...)):
    ...
    try:
        ...
        recept_data = _verwerk_recept_data(json.loads(tekst))
...
async def _bereken_order_details(data: dict, wc_url: str, wc_key: str, wc_secret: str) -> dict:
    """
    Berekent alles wat nodig is voor een order (product-ID, aantal eenheden,
    line_items, korting) — GEEN aanroep naar WooCommerce om de order zelf aan
    te maken. Gedeeld door maak_order() en het preview-order-endpoint, zodat
    een preview gegarandeerd exact dezelfde uitkomst laat zien als de
    daadwerkelijke aanmaak — geen los, mogelijk afwijkend rekenpad.
    """
    ...
    getal_match = _re.search(r"[\d.]+", hoeveelheid_str.replace(",", "."))
    ...
    return {
        "medicijn": medicijn, "product_id": product_id,
        "aantal_tubes": aantal_tubes, "verpakkingsgrootte": verpakkingsgrootte,
        "billing": billing, "line_items": line_items, "oorsprong": oorsprong,
        "coulance_bedrag": coulance_bedrag, "coulance_totaal": korting_totaal,
        "voornaam": voornaam, "achternaam": achternaam,
    }


@app.post("/api/preview-order")
async def preview_order(request: Request):
    """
    Toont precies wat er zou gebeuren bij het aanmaken van een order —
    zonder de order daadwerkelijk aan te maken. Gebruikt dezelfde
    berekeningsfunctie als maak_order(), dus gegarandeerd dezelfde uitkomst
    als de daadwerkelijke aanmaak. Haalt alleen (read-only) prijzen op bij
    WooCommerce om een correct totaalbedrag te kunnen tonen.
    """
    data = await request.json()
    ...
    berekening = await _bereken_order_details(data, wc_url, wc_key, wc_secret)

    regels = []
    totaal = 0.0
    for item in berekening["line_items"]:
        pid = item["product_id"]
        qty = item["quantity"]
        if "total" in item:
            # Korting: prijs is al expliciet ingesteld, geen catalogusprijs nodig
            bedrag = float(item["total"])
            naam = "Coulanceregeling per tube"
        else:
            naam, prijs_per_stuk = f"Product #{pid}", 0.0
            try:
                resp = http_requests.get(f"{wc_url}/wp-json/wc/v3/products/{pid}", headers=_wc_auth(wc_key, wc_secret), timeout=8)
                if resp.status_code == 200:
                    p = resp.json()
                    naam = p.get("name", naam)
                    prijs_per_stuk = float(p.get("price") or 0)
            except Exception:
                pass
            bedrag = round(prijs_per_stuk * qty, 2)
        regels.append({"naam": naam, "product_id": pid, "aantal": qty, "bedrag": round(bedrag, 2)})
        totaal += bedrag

    return JSONResponse(content={
        "medicijn": berekening["medicijn"],
        "medicijn_gevonden": berekening["product_id"] is not None,
        "aantal_tubes": berekening["aantal_tubes"],
        "verpakkingsgrootte": berekening["verpakkingsgrootte"],
        "patient_naam": f"{berekening['voornaam']} {berekening['achternaam']}".strip(),
        "email": berekening["billing"]["email"],
        "coulance_van_toepassing": berekening["coulance_bedrag"] > 0,
        "coulance_bedrag_per_tube": berekening["coulance_bedrag"],
        "regels": regels,
...
async def maak_order(request: Request):
    ...
    berekening = await _bereken_order_details(data, wc_url, wc_key, wc_secret)
    medicijn = berekening["medicijn"]
    product_id = berekening["product_id"]
    billing = berekening["billing"]
    line_items = berekening["line_items"]
    oorsprong = berekening["oorsprong"]
    ...
    try:
        ...
        if not response.ok:
            # De generieke 'raise_for_status()'-tekst ("400 Client Error: ...")
            # verbergt de eigenlijke reden die WooCommerce teruggeeft (bijv.
            # een ongeldig product-ID of een afgewezen veld) — die WEL tonen,
            # zodat dit niet elke keer via de server-log hoeft te worden opgezocht.
            try:
                wc_detail = response.json().get("message", response.text[:300])
            except Exception:
                wc_detail = response.text[:300]
            return JSONResponse(content={"fout": f"WooCommerce ({response.status_code}): {wc_detail}"})
        order = response.json()
...
async def verstuur_order_naar_klant(order_id: int):
    """
    Verstuurt de bestelgegevens naar de klant, ZONDER de orderstatus te
    wijzigen — de klant moet mogelijk nog betalen, dus een order naar
    'processing' zetten puur om een e-mail te versturen zou (nu we
    'processing' expliciet als 'betaald' behandelen, zie de
    betalingen-verwerken-flow) een verkeerd signaal afgeven.

    Probeert eerst het speciale WooCommerce-endpoint hiervoor
    (POST .../actions/send_order_details, beschikbaar vanaf WooCommerce 9.5)
    — valt terug op een klantnotitie (die WooCommerce automatisch als e-mail
    verstuurt) als dat endpoint niet bestaat op een oudere WooCommerce-versie.
    """
    wc_url = os.getenv("WC_URL", ""); wc_key = os.getenv("WC_KEY", ""); wc_secret = os.getenv("WC_SECRET", "")
    if not (wc_url and wc_key and wc_secret):
    ...
    try:
        resp = http_requests.post(
            f"{wc_url}/wp-json/wc/v3/orders/{order_id}/actions/send_order_details",
            headers=_wc_auth(wc_key, wc_secret),
            timeout=15,
        )
        if resp.status_code in (200, 201):
            return JSONResponse(content={"ok": True, "methode": "send_order_details"})
        if resp.status_code != 404:
            # Endpoint bestaat wel, maar iets anders ging mis — niet stilzwijgend
            # doorvallen naar de terugval, want dat zou de echte fout verbergen.
            return JSONResponse(status_code=500, content={"ok": False, "fout": f"HTTP {resp.status_code}: {resp.text[:200]}"})
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "fout": str(e)})

    # Terugval: oudere WooCommerce zonder send_order_details — een klantnotitie
    # activeert WooCommerce's eigen 'Notitie van de klant'-e-mail, ook zonder statuswijziging.
    try:
        resp2 = http_requests.post(
            f"{wc_url}/wp-json/wc/v3/orders/{order_id}/notes",
            headers=_wc_auth(wc_key, wc_secret),
            json={
                "note": "Uw bestelgegevens: zie het overzicht in uw account of neem contact op voor details.",
                "customer_note": True,
            },
            timeout=15,
        )
        resp2.raise_for_status()
        return JSONResponse(content={"ok": True, "methode": "klantnotitie (terugval)"})
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "fout": str(e)})
...
def _is_order_verzonden(oid, wc_url: str, wc_key: str, wc_secret: str) -> tuple[bool, str]:
    """
    Checkt of een order al verzonden is, via dezelfde detectie als
    /api/verzend-status: een ordernotitie die 'sendcloud', 'postnl' of
    'tracking' bevat, mét een URL erin. Geeft (verzonden, tracking_url) terug.
    """
    import re as _re_verzonden
    try:
        resp = http_requests.get(
        ...
        if resp.status_code == 200:
            for note in resp.json():
                ...
                if "sendcloud" in note_tekst.lower() or "postnl" in note_tekst.lower() or "tracking" in note_tekst.lower():
                    url_match = _re_verzonden.search(r'(https?://\S+)', note_tekst)
                    if url_match:
                        return True, url_match.group(1).replace("&amp;", "&").rstrip(".")
    except Exception:
        pass
    return False, ""
...
async def betalingen_verwerken(request: Request):
    ...
    Zet naar 'processing' (betaald, nog niet verzonden), of direct naar
    'completed' als er al een SendCloud-verzendnotitie op de order staat
    (betaald én verzonden — beide voorwaarden voor volledig afgerond).
    """
    body = await request.json()
    ...
    for oid in order_ids:
        try:
            verzonden, _ = _is_order_verzonden(oid, wc_url, wc_key, wc_secret)
            nieuwe_status = "completed" if verzonden else "processing"
            ...
            resultaten.append({"order_id": oid, "ok": resp.status_code == 200, "status": nieuwe_status})
        except Exception as e:
...
async def haal_verzend_status(request: Request):
    ...
    resultaten = {}
    for oid in order_ids:
        verzonden, tracking_url = _is_order_verzonden(oid, wc_url, wc_key, wc_secret)
        resultaten[str(oid)] = {"verzonden": verzonden, "tracking_url": tracking_url}
...
async def analyseer_order(request: Request):
    ...
    # Bestandstype aan de werkelijke bytes herkennen (niet aan de URL-
    # extensie) — een telefoon-screenshot heeft zelden een .pdf-extensie én
    # is zelden JPEG, meestal PNG. Het verkeerde media_type opgeven laat
    # Claude's API het verzoek afwijzen met een 400 Bad Request.
    media_type = _detecteer_bestandstype(recept_bytes)
    if media_type == "application/octet-stream":
        return JSONResponse(content={"fout": "Onherkenbaar bestandstype (geen PDF, JPEG, PNG of GIF) — kan dit recept niet uitlezen."})
...
def _vergelijk_order_recept(wc_order: dict, recept: dict) -> dict:
    ...
    # Losse, expliciete waarschuwing bij een naam-mismatch — dit is het
    # belangrijkste signaal dat een recept van een ANDERE patiënt is
    # geüpload, en verdient meer dan alleen een lager totaalpercentage
    # (dat kan ook door kleine OCR-afwijkingen op andere velden komen).
    if veld_naam["score"] < 60:
        aandachtspunten.append(
            f"⛔ Naam op recept ({recept.get('patient_naam') or '—'}) komt niet overeen met "
            f"de bestelling ({wc_naam or '—'}) — controleer of dit recept wel bij deze patiënt hoort"
        )
...
@app.post("/api/order/{order_id}/vraag-recept-op")
async def vraag_recept_op(order_id: int, request: Request):
    """
    Voor het pad 'onbekende klant, geen recept aangeleverd': er is geen
    enkele bron (geen recept, geen bekend dossier) om de order op te
    verwerken. Vraagt het recept alsnog op bij de klant — via een
    klantnotitie (net als /api/order/{id}/verstuur-naar-klant hierboven),
    zodat WooCommerce automatisch een e-mail stuurt, geen aparte SMTP-code
    nodig — en zet de order tegelijk op 'on-hold': deze staat na betaling op
    'processing' ('in behandeling'), en hoort niet zo te blijven staan
    terwijl we op het recept wachten.
    """
    wc_url = os.getenv("WC_URL", ""); wc_key = os.getenv("WC_KEY", ""); wc_secret = os.getenv("WC_SECRET", "")
    if not (wc_url and wc_key and wc_secret):
        return JSONResponse(status_code=500, content={"ok": False, "fout": "WooCommerce niet geconfigureerd"})
    ...
    body = await request.json()
    bericht = (body.get("bericht") or "").strip()
    if not bericht:
        return JSONResponse(status_code=400, content={"ok": False, "fout": "Geen berichttekst opgegeven"})

    try:
        resp_note = http_requests.post(
            f"{wc_url}/wp-json/wc/v3/orders/{order_id}/notes",
            headers=_wc_auth(wc_key, wc_secret),
            json={"note": bericht, "customer_note": True},
            timeout=15,
        )
        resp_note.raise_for_status()
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "fout": f"Versturen mislukt: {e}"})

    try:
        resp_status = http_requests.put(
            f"{wc_url}/wp-json/wc/v3/orders/{order_id}",
            headers=_wc_auth(wc_key, wc_secret),
            json={"status": "on-hold"},
            timeout=15,
        )
        status_ok = resp_status.status_code == 200
    except Exception as e:
        # De e-mail is al onderweg — dat niet als fout terugmelden, alleen de
        # (niet-kritieke) statuswijziging expliciet als mislukt markeren.
        return JSONResponse(content={"ok": True, "verstuurd": True, "on_hold": False, "fout_status": str(e)})

    return JSONResponse(content={"ok": True, "verstuurd": True, "on_hold": status_ok})


# ------------------------------------------------------------------
# Stap 5: standaard (deels gepersonaliseerde) statusmail naar de klant
# ------------------------------------------------------------------

# Eén van deze vijf scenario's wordt gekozen (dropdown in de front-end) nadat
# de bestelling met het recept is vergeleken. De tekst hieronder vormt het
# "lichaam" van de e-mail, ná de vaste aanhef + "U heeft X besteld"-regel —
# zie _scenario_email_body() hieronder. Vrij aan te passen: dit is platte
# tekst, geen code.
SCENARIO_EMAIL_TEKSTEN = {
    1: (  # Nieuwe patiënt, geldig recept aangeleverd
        "Wij hebben uw recept in goede orde ontvangen en gecontroleerd. "
        "Uw bestelling wordt nu door onze apotheek in behandeling genomen. "
        "Zodra uw medicatie is verzonden, ontvangt u hiervan een aparte bevestiging."
    ),
    2: (  # Nieuwe patiënt, geen (geldig) recept aangeleverd
        "Om uw bestelling te kunnen verwerken hebben wij nog een geldig recept "
        "van u nodig. U kunt dit eenvoudig aanleveren door te antwoorden op deze "
        "e-mail met een duidelijke foto of scan van uw recept. Zodra wij uw "
        "recept hebben ontvangen en gecontroleerd, pakken wij uw bestelling direct op."
    ),
    3: (  # Bekende patiënt, geldig herhaalrecept
        "Wij hebben uw herhaalrecept gecontroleerd en dit is nog geldig. "
        "Uw bestelling wordt nu door onze apotheek in behandeling genomen."
    ),
    4: (  # Bekende patiënt, geen geldig herhaalrecept (of verlopen)
        "Bij controle blijkt dat wij op dit moment geen geldig herhaalrecept "
        "van u hebben, of dat uw recept inmiddels is verlopen. Om uw bestelling "
        "te kunnen verwerken vragen wij u een nieuw recept aan te leveren, "
        "bijvoorbeeld via uw voorschrijver of huisarts."
    ),
    5: (  # Bestelling en recept komen niet overeen
        "Bij het controleren van uw bestelling zien wij een verschil tussen de "
        "gegevens op uw recept en de gegevens van uw bestelling (zoals naam, "
        "medicijn of hoeveelheid). Om misverstanden te voorkomen nemen wij "
        "hierover eerst contact met u op voordat wij uw bestelling verder verwerken."
    ),
}


def _scenario_email_body(voornaam: str, medicijn_naam: str, scenario: int) -> str:
    """Bouwt de volledige mailtekst: aanhef + 'u heeft besteld' + standaardtekst."""
    aanhef = f"Beste {voornaam}," if voornaam else "Beste klant,"
    regel_bestelling = f"U heeft {medicijn_naam} bij ons besteld." if medicijn_naam else "Wij hebben uw bestelling ontvangen."
    standaardtekst = SCENARIO_EMAIL_TEKSTEN.get(scenario, "")
    return f"{aanhef}\n\n{regel_bestelling}\n\n{standaardtekst}\n\nMet vriendelijke groet,\nFarmamed"


def _verstuur_email_info_farmamed(naar: str, onderwerp: str, tekst: str) -> None:
    """
    Verstuurt een e-mail rechtstreeks via het info@farmamed.nl-mailaccount,
    zodat het afzenderadres altijd info@farmamed.nl is — in plaats van welk
    'systeem-van'-adres er ook in WooCommerce/WordPress staat ingesteld (de
    vorige aanpak, via een WooCommerce-klantnotitie, liet WordPress bepalen
    wie de afzender was).

    Verwacht dezelfde soort cPanel/Dovecot-mailaccount als IMAP_SERVER/
    IMAP_USER hierboven (SMTP is bij vrijwel elke hostingpartij gewoon
    hetzelfde account/wachtwoord als IMAP, alleen een andere poort).

    Benodigde environment variables (Railway):
      SMTP_HOST  — bijv. mail.farmamed.nl of smtp.jouw-hostingpartij.nl
      SMTP_PORT  — 465 (SSL, standaard) of 587 (STARTTLS)
      SMTP_USER  — standaard info@farmamed.nl, override mogelijk
      SMTP_PASS  — wachtwoord van het info@farmamed.nl-mailaccount

    Gooit een Exception door bij een configuratie- of verzendfout — de
    aanroeper vertaalt dat naar een nette foutmelding voor de gebruiker.
    """
    import smtplib
    ...
    smtp_user = os.getenv("SMTP_USER", "info@farmamed.nl")
    smtp_pass = os.getenv("SMTP_PASS", "")
    if not (smtp_host and smtp_pass):
        raise RuntimeError("SMTP niet geconfigureerd (SMTP_HOST/SMTP_PASS ontbreken in de environment variables)")

    msg = MIMEText(tekst, "plain", "utf-8")
    msg["Subject"] = onderwerp
    msg["From"] = formataddr(("Farmamed", smtp_user))
    msg["To"] = naar

    if smtp_port == 465:
        with smtplib.SMTP_SSL(smtp_host, smtp_port, timeout=15) as server:
            server.login(smtp_user, smtp_pass)
            server.sendmail(smtp_user, [naar], msg.as_string())
    else:
...
@app.post("/api/order/{order_id}/verstuur-scenario-mail")
async def verstuur_scenario_mail(order_id: int, request: Request):
    """
    Stap 5 van de orderstroom: verstuurt een standaard, deels gepersonaliseerde
    e-mail naar de klant, gekozen via een dropdown met 5 scenario's:
      1 = nieuwe patiënt, geldig recept
      2 = nieuwe patiënt, geen geldig recept
      3 = bekende patiënt, geldig herhaalrecept
      4 = bekende patiënt, geen geldig (of verlopen) herhaalrecept
      5 = bestelling en recept komen niet overeen

    Haalt naam/e-mail/medicijn rechtstreeks uit de WooCommerce-order op
    (alleen om de gegevens op te zoeken — dit werkt ongeacht orderstatus of
    of er al een SmartHUB-JSON is aangemaakt/verstuurd; stap 5 staat los van
    stap 4), bouwt de mailtekst op (aanhef + 'u heeft besteld' + standaard-
    tekst bij het scenario) en verstuurt die zelf via SMTP vanaf
    info@farmamed.nl — zie _verstuur_email_info_farmamed hierboven.
    Wijzigt de orderstatus NIET — dat blijft stap 6 (afronden).
    """
    body = await request.json()
    scenario = body.get("scenario")
    medicijn_naam_override = (body.get("medicijn_naam") or "").strip()

    if scenario not in SCENARIO_EMAIL_TEKSTEN:
        return JSONResponse(status_code=400, content={"ok": False, "fout": "Ongeldig scenario (verwacht 1 t/m 5)"})
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"ok": False, "fout": f"Kon order niet ophalen: {e}"})

    billing = order.get("billing", {})
    voornaam = billing.get("first_name", "").strip()
    klant_email = billing.get("email", "").strip()
    items = order.get("line_items", [])
    medicijn_naam = medicijn_naam_override or (items[0]["name"] if items else "")

    if not klant_email:
        return JSONResponse(status_code=400, content={"ok": False, "fout": "Geen e-mailadres bekend op deze order"})

    mailtekst = _scenario_email_body(voornaam, medicijn_naam, scenario)
    onderwerp = f"Farmamed — uw bestelling #{order_id}"

    try:
        _verstuur_email_info_farmamed(klant_email, onderwerp, mailtekst)
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "fout": f"Versturen mislukt: {e}"})

    return JSONResponse(content={
        "ok": True, "verstuurd": True, "scenario": scenario,
        "naar": klant_email, "medicijn_naam": medicijn_naam, "mailtekst": mailtekst,
    })


@app.get("/admin/dubbele-patienten", response_class=HTMLResponse)
async def admin_dubbele_patienten():
    """Simpel uploadschermpje voor /api/dubbele-patienten/samenvoegen — geen curl/Postman nodig."""
    return HTMLResponse(content="""<!DOCTYPE html>
<html lang="nl"><head><meta charset="UTF-8">
<title>Dubbele patiënten samenvoegen</title>
<style>
  body { font-family: 'DM Sans', Arial, sans-serif; max-width: 720px; margin: 40px auto; padding: 0 20px; color: #2c2c2c; }
  h1 { font-size: 22px; font-weight: 600; }
  p { color: #555; line-height: 1.5; }
  .kaart { background: #fff; border: 1px solid #ddd; border-radius: 10px; padding: 20px; margin-top: 20px; }
  input[type=file] { display: block; margin: 12px 0; }
  button { padding: 10px 18px; background: #5a9898; color: white; border: none; border-radius: 8px;
           font-size: 14px; cursor: pointer; font-weight: 600; }
  button:disabled { background: #aac; cursor: not-allowed; }
  table { width: 100%; border-collapse: collapse; margin-top: 16px; font-size: 13px; }
  th, td { text-align: left; padding: 6px 8px; border-bottom: 1px solid #eee; }
  th { background: #e8f4f4; }
  .ok { color: #2a9d5c; font-weight: 600; }
  .skip { color: #b8860b; font-weight: 600; }
  .fout { color: #c0392b; font-weight: 600; }
  #samenvatting { font-weight: 600; margin-top: 16px; }
</style></head>
<body>
  <h1>Dubbele patiënten samenvoegen</h1>
  <p>Upload hier het (volledig ingevulde, met 'Ja'/'Nee' op tabblad 'DUBBELINGEN' en
  'Placeholder → kandidaat') rapport. Dit voert de samenvoegingen direct door op de database —
  er is geen aparte controlestap.</p>
  <div class="kaart">
    <input type="file" id="bestand" accept=".xlsx">
    <button onclick="samenvoegen()" id="btn">Samenvoegen uitvoeren</button>
...
async function samenvoegen() {
  ...
  if (!input.files[0]) { alert('Kies eerst een bestand.'); return; }
  btn.disabled = true; btn.textContent = 'Bezig…';
  resultaat.innerHTML = '';
  const formData = new FormData();
  formData.append('bestand', input.files[0]);
  try {
    const res = await fetch('/api/dubbele-patienten/samenvoegen', { method: 'POST', body: formData });
    const data = await res.json();
    if (data.fout) {
      resultaat.innerHTML = '<p class="fout">Fout: ' + data.fout + '</p>';
      return;
    }
    let html = `<div id="samenvatting">${data.samengevoegd} samengevoegd, ${data.overgeslagen} overgeslagen, ${data.mislukt} mislukt (van ${data.totaal_groepen} groepen)</div>`;
    html += '<table><tr><th>Leden</th><th>Status</th><th>Details</th></tr>';
    data.resultaten.forEach(r => {
      let klasse = r.status === 'samengevoegd' ? 'ok' : (r.status === 'overgeslagen' ? 'skip' : 'fout');
      let details = '';
      if (r.status === 'samengevoegd') {
        details = `hoofd: ${r.hoofd_patient_id}, e-mail: ${r.gekozen_email}, orders verplaatst: ${r.orders_verplaatst}`;
      } else if (r.status === 'overgeslagen') {
        details = r.reden;
      } else {
        details = r.fout;
      }
      html += `<tr><td>${r.leden.join(', ')}</td><td class="${klasse}">${r.status}</td><td>${details}</td></tr>`;
    });
    html += '</table>';
    resultaat.innerHTML = html;
  } catch(e) {
    resultaat.innerHTML = '<p class="fout">Onverwachte fout: ' + e.message + '</p>';
  } finally {
    btn.disabled = false; btn.textContent = 'Samenvoegen uitvoeren';
  }
}
</script>
</body></html>""")
...
@app.post("/api/dubbele-patienten/samenvoegen")
async def samenvoeg_dubbele_patienten(bestand: UploadFile = File(...)):
    """
    Voert de in /api/rapport/dubbele-patienten bevestigde samenvoegingen
    daadwerkelijk door. Verwacht hetzelfde .xlsx-bestand terug, met op
    tabblad 'DUBBELINGEN' de kolom 'Dubbel' en op tabblad
    'Placeholder → kandidaat' de kolom 'MATCH?' ingevuld met 'Ja'/'Nee' —
    en eventueel het E-mail-veld gecorrigeerd naar het juiste adres (dat
    gecorrigeerde adres wordt als bron van waarheid gebruikt).

    Per groep (patiënten die via tabblad 1 en/of 2 met 'Ja' aan elkaar
    gekoppeld zijn — ook transitief over beide tabbladen heen, want een
    patiënt kan in allebei voorkomen):
      - De patiënt met de meeste orders wordt de hoofdpatiënt (bij een
        gelijke stand: laagste patiënt-ID, dus het oudste dossier).
      - Het e-mailadres: als alle niet-placeholder e-mails in de groep het
        eens zijn, of er een duidelijke meerderheid is, wordt die gebruikt.
        Bij een gelijke stand wordt die ENE groep overgeslagen (blijft
        ongewijzigd) — geen giswerk op een e-mailadres.
      - Ontbrekende velden op de hoofdpatiënt (bsn, geboortedatum, telefoon,
        adres, postcode, plaats) worden aangevuld vanuit de andere leden.
      - orders.patient_id ÉN whatsapp_verzoeken.patient_id van de andere
        leden gaan naar de hoofdpatiënt — er gaat dus geen order- of
        WhatsApp-geschiedenis verloren, alleen de dossier-koppeling
        verandert. WooCommerce zelf wordt niet aangeraakt (orders zijn daar
        losse gast-orders, zie eerdere toelichting).
      - De andere (nu lege) patiëntrijen worden verwijderd.

    Elke groep in een EIGEN transactie: gaat er bij één groep iets mis, dan
    blijven de al gelukte groepen gewoon staan.
    """
    import openpyxl
    ...
    inhoud = await bestand.read()
    try:
        wb = openpyxl.load_workbook(BytesIO(inhoud), data_only=True)
    except Exception as e:
        return JSONResponse(status_code=400, content={"fout": f"Kon het bestand niet lezen: {e}"})

    if "DUBBELINGEN" not in wb.sheetnames or "Placeholder → kandidaat" not in wb.sheetnames:
        return JSONResponse(status_code=400, content={
            "fout": "Verwacht tabbladen 'DUBBELINGEN' en 'Placeholder → kandidaat' — is dit het rapport uit /api/rapport/dubbele-patienten?"
        })

    # --- Groepen + gekozen e-mail per patiënt-ID uit het bestand halen ---
    email_per_id = {}
    koppelingen = []

    ws1 = wb["DUBBELINGEN"]
    groepen_tab1 = defaultdict(list)
    for row in ws1.iter_rows(min_row=2, values_only=True):
        if not row or row[0] is None:
            continue
        groep, dubbel, pid = row[0], row[1], row[2]
        email_per_id[pid] = (row[7] or "").strip()
        if dubbel and str(dubbel).strip().lower() == "ja":
            groepen_tab1[groep].append(pid)
    for leden in groepen_tab1.values():
        for i in range(1, len(leden)):
            koppelingen.append((leden[0], leden[i]))

    ws2 = wb["Placeholder → kandidaat"]
    for row in ws2.iter_rows(min_row=2, values_only=True):
        if not row or row[3] is None:
            continue
        match, pid_ph, pid_k, email_k = row[2], row[3], row[9], row[11]
        email_per_id.setdefault(pid_k, (email_k or "").strip())
        if match and str(match).strip().lower() == "ja":
            koppelingen.append((pid_ph, pid_k))

    if not koppelingen:
        return JSONResponse(content={"fout": "Geen enkele rij met 'Ja' gevonden — niets om samen te voegen."})

    alle_ids = set()
    for a, b in koppelingen:
        alle_ids.add(a); alle_ids.add(b)

    parent = {pid: pid for pid in alle_ids}
    ...
    for pid in alle_ids:
        groepen_dict[_vind(pid)].append(pid)
    groepen = list(groepen_dict.values())

    PLACEHOLDER_DOMEINEN = ("@farmamed.nl", "@apotheekwoerden.nl")

    def _is_placeholder(email):
        e = (email or "").strip().lower()
        return not e or e.endswith(PLACEHOLDER_DOMEINEN)

    # --- Uitvoeren, per groep een eigen transactie ---
    resultaten = []
    for leden_ids in groepen:
        leden_ids = sorted(set(leden_ids))
        conn = _pg_conn()
        cur = conn.cursor()
        try:
            cur.execute(
                "SELECT id, email, voornaam, achternaam, geboortedatum, bsn, telefoon, adres, postcode, plaats, "
                "(SELECT COUNT(*) FROM orders o WHERE o.patient_id = p.id) AS aantal_orders "
                "FROM patienten p WHERE id = ANY(%s)",
                (leden_ids,),
            )
            kolommen = ["id", "email", "voornaam", "achternaam", "geboortedatum", "bsn", "telefoon",
                        "adres", "postcode", "plaats", "aantal_orders"]
            leden = [dict(zip(kolommen, r)) for r in cur.fetchall()]

            if len(leden) < 2:
                resultaten.append({"leden": leden_ids, "status": "overgeslagen",
                                    "reden": "een of meer patiënt-ID's bestaan niet meer in de database"})
                conn.rollback(); cur.close(); conn.close()
                continue

            canonical = sorted(leden, key=lambda p: (-p["aantal_orders"], p["id"]))[0]

            echte_emails = [email_per_id.get(l["id"], l["email"]) for l in leden]
            echte_emails = [e for e in echte_emails if not _is_placeholder(e)]
            if not echte_emails:
                gekozen_email = canonical["email"] or ""
            else:
                telling = defaultdict(int)
                for e in echte_emails:
                    telling[e.lower()] += 1
                gesorteerd = sorted(telling.items(), key=lambda kv: -kv[1])
                if len(gesorteerd) >= 2 and gesorteerd[0][1] == gesorteerd[1][1]:
                    resultaten.append({"leden": leden_ids, "status": "overgeslagen",
                                        "reden": f"gelijke stand tussen e-mailadressen: {[e for e, _ in gesorteerd]}"})
                    conn.rollback(); cur.close(); conn.close()
                    continue
                gekozen_email = next(e for e in echte_emails if e.lower() == gesorteerd[0][0])

            andere_ids = [l["id"] for l in leden if l["id"] != canonical["id"]]

            aanvullingen = {}
            for veld in ("voornaam", "achternaam", "geboortedatum", "bsn", "telefoon", "adres", "postcode", "plaats"):
                if not canonical.get(veld):
                    for l in leden:
                        if l.get(veld):
                            aanvullingen[veld] = l[veld]
                            break

            zet_velden = {**aanvullingen, "email": gekozen_email}
            set_clause = ", ".join(f"{k} = %s" for k in zet_velden)
            cur.execute(f"UPDATE patienten SET {set_clause} WHERE id = %s",
                        (*zet_velden.values(), canonical["id"]))

            cur.execute("UPDATE orders SET patient_id = %s WHERE patient_id = ANY(%s)",
                        (canonical["id"], andere_ids))
            verplaatste_orders = cur.rowcount
            cur.execute("UPDATE whatsapp_verzoeken SET patient_id = %s WHERE patient_id = ANY(%s)",
                        (canonical["id"], andere_ids))

            cur.execute("DELETE FROM patienten WHERE id = ANY(%s)", (andere_ids,))

            conn.commit()
            resultaten.append({
                "leden": leden_ids, "status": "samengevoegd", "hoofd_patient_id": canonical["id"],
                "verwijderde_ids": andere_ids, "gekozen_email": gekozen_email,
                "aangevulde_velden": aanvullingen, "orders_verplaatst": verplaatste_orders,
            })
        except Exception as e:
            conn.rollback()
            resultaten.append({"leden": leden_ids, "status": "mislukt", "fout": str(e)})
        finally:
            cur.close(); conn.close()

    return JSONResponse(content={
        "totaal_groepen": len(groepen),
        "samengevoegd": len([r for r in resultaten if r["status"] == "samengevoegd"]),
        "overgeslagen": len([r for r in resultaten if r["status"] == "overgeslagen"]),
        "mislukt": len([r for r in resultaten if r["status"] == "mislukt"]),
        "resultaten": resultaten,
    })


@app.post("/api/voorschrijvers-koppelen/rapport")
async def voorschrijvers_koppelen_rapport(bestand: UploadFile = File(...)):
    """
    Rapport (wijzigt niets in de database). Verwacht een .xlsx met een
    tabblad 'Export' en kolommen PrescriberName, DateofBirth, Initials,
    Surname, Contact[Phone], Contact[Email], Contact[Address] — elke rij
    koppelt een patiënt (geboortedatum/initialen/achternaam/contact) aan
    diens voorschrijver (PrescriberName).

    Matcht elke rij tegen onze eigen patiënten-database met dezelfde
    beproefde matching als de patiëntherkenning elders in de app
    (_zoek_patient_kandidaten — BSN/e-mail/naam+geboortedatum). Download het
    resultaat, controleer/corrigeer de 'Actie'-kolom (Ja/Nee — al voorgesteld
    bij een eenduidige match), en upload het terug naar
    /api/voorschrijvers-koppelen/uitvoeren om de voorschrijvers daadwerkelijk
    ...
    inhoud = await bestand.read()
    try:
        wb_in = openpyxl.load_workbook(BytesIO(inhoud), data_only=True)
    except Exception as e:
        return JSONResponse(status_code=400, content={"fout": f"Kon het bestand niet lezen: {e}"})

    if "Export" not in wb_in.sheetnames:
        return JSONResponse(status_code=400, content={"fout": "Verwacht een tabblad 'Export' — is dit het juiste bestand?"})
    ws_in = wb_in["Export"]
    ...
    for row in ws_in.iter_rows(min_row=2, values_only=True):
        if not row or not row[0]:
            continue
        prescriber_naam, geboortedatum, initials, surname, phone, email, adres = (list(row) + [None] * 7)[:7]
        geboortedatum_str = geboortedatum.strftime("%d-%m-%Y") if hasattr(geboortedatum, "strftime") else str(geboortedatum or "").strip()
        # Contact[Phone] kan meerdere, komma-gescheiden nummers bevatten met
        # een 'Personal :'/'Other :'-prefix — eerste nummer eruit halen.
        telefoon = ""
        if phone:
            eerste = str(phone).split(",")[0]
            telefoon = eerste.split(":")[-1].strip() if ":" in eerste else eerste.strip()
        rijen.append({
            "voorschrijver_naam": (prescriber_naam or "").strip(),
    ...
    resultaten = []
    for r in rijen:
        if r["voorschrijver_naam"].upper() == "SPECIALIST":
            resultaten.append({**r, "match_status": "overgeslagen", "reden": "'SPECIALIST' is een generieke categorie, geen naam",
                                "patient_id": "", "patient_naam_db": "", "actie": ""})
            continue
        try:
            kandidaten = await _zoek_patient_kandidaten(
                email=r["email"], naam=r["surname"], telefoon=r["telefoon"],
                geboortedatum=r["geboortedatum"], max_kandidaten=3,
            )
        except Exception as e:
            resultaten.append({**r, "match_status": "fout bij zoeken", "reden": str(e),
                                "patient_id": "", "patient_naam_db": "", "actie": ""})
            continue

        if len(kandidaten) == 1:
            k = kandidaten[0]
            resultaten.append({**r, "match_status": "eenduidig", "reden": f'{k.get("match_methode","")} ({k.get("match_score","")}%)',
                                "patient_id": k["id"], "patient_naam_db": f'{k.get("voornaam","")} {k.get("achternaam","")}'.strip(),
                                "actie": "Ja"})
        elif len(kandidaten) > 1:
            for k in kandidaten:
                resultaten.append({**r, "match_status": f"meerdere kandidaten ({len(kandidaten)})",
                                    "reden": f'{k.get("match_methode","")} ({k.get("match_score","")}%)',
                                    "patient_id": k["id"], "patient_naam_db": f'{k.get("voornaam","")} {k.get("achternaam","")}'.strip(),
                                    "actie": ""})
        else:
            resultaten.append({**r, "match_status": "geen match", "reden": "",
                                "patient_id": "", "patient_naam_db": "", "actie": ""})

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Voorschrijvers koppelen"
    headers = ["Actie (Ja/Nee)", "Match-status", "Match-reden", "Patient ID", "Patiëntnaam in database",
               "Voorschrijver (uit bestand)", "Geboortedatum", "Initialen pat.", "Achternaam pat.",
               "Telefoon pat.", "E-mail pat.", "Adres pat."]
    ws.append(headers)
    for cel in ws[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="5A9898")
        cel.alignment = Alignment(horizontal="left")
    for r in resultaten:
        ws.append([r["actie"], r["match_status"], r["reden"], r["patient_id"], r["patient_naam_db"],
                   r["voorschrijver_naam"], r["geboortedatum"], r["initials"], r["surname"],
                   r["telefoon"], r["email"], r["adres"]])
    for kolom, breedte in zip("ABCDEFGHIJKL", [12, 22, 26, 10, 22, 20, 13, 10, 16, 14, 24, 32]):
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=voorschrijvers_koppelen_rapport.xlsx"},
    )


@app.post("/api/voorschrijvers-koppelen/uitvoeren")
async def voorschrijvers_koppelen_uitvoeren(bestand: UploadFile = File(...)):
    """
    Voert de in het rapport bevestigde koppelingen ('Ja' in de Actie-kolom)
    daadwerkelijk door: zet patienten.voorschrijver_id (en voorschrijver_datum)
    voor elke bevestigde rij. Eén voorschrijver-naam wordt maar één keer
    aangemaakt/opgezocht (lokale cache in dit verzoek), ook als dezelfde
    voorschrijver bij tientallen patiënten voorkomt.
    """
    import openpyxl
    from io import BytesIO
    ...
    ws = wb["Voorschrijvers koppelen"] if "Voorschrijvers koppelen" in wb.sheetnames else wb.active
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        if not row or len(row) < 6:
            continue
        actie, status, reden, patient_id, patient_naam_db, voorschrijver_naam = row[:6]
        if not actie or str(actie).strip().lower() != "ja":
            continue
        if not patient_id:
            resultaten.append({"patient_id": None, "status": "mislukt", "fout": "geen patient_id in deze rij"})
            continue
        naam = (voorschrijver_naam or "").strip()
        if not naam:
            resultaten.append({"patient_id": patient_id, "status": "mislukt", "fout": "geen voorschrijvernaam in deze rij"})
            continue

        if naam not in voorschrijver_cache:
            voorschrijver_cache[naam] = _zoek_of_maak_voorschrijver(naam, "")
        voorschrijver_id = voorschrijver_cache[naam]

        conn = _pg_conn()
        cur = conn.cursor()
        try:
            cur.execute(
                "UPDATE patienten SET voorschrijver_id = %s, voorschrijver_datum = NOW() WHERE id = %s",
                (voorschrijver_id, int(patient_id)),
            )
            conn.commit()
            resultaten.append({"patient_id": patient_id, "voorschrijver_id": voorschrijver_id,
                                "voorschrijver_naam": naam, "status": "bijgewerkt"})
        except Exception as e:
            conn.rollback()
            resultaten.append({"patient_id": patient_id, "status": "mislukt", "fout": str(e)})
        finally:
            cur.close()
            conn.close()
    ...
    return JSONResponse(content={
        ...
        "bijgewerkt": len([r for r in resultaten if r["status"] == "bijgewerkt"]),
        "mislukt": len([r for r in resultaten if r["status"] == "mislukt"]),
        "unieke_voorschrijvers": len(voorschrijver_cache),
        "resultaten": resultaten,
    })


@app.get("/admin/voorschrijvers-koppelen", response_class=HTMLResponse)
async def admin_voorschrijvers_koppelen():
    """Uploadschermpje voor de twee routes hierboven — rapport genereren, dan (na controle) uitvoeren."""
    return HTMLResponse(content="""<!DOCTYPE html>
<html lang="nl"><head><meta charset="UTF-8">
<title>Voorschrijvers koppelen</title>
<style>
  body { font-family: 'DM Sans', Arial, sans-serif; max-width: 720px; margin: 40px auto; padding: 0 20px; color: #2c2c2c; }
  h1 { font-size: 22px; font-weight: 600; }
  ...
  .ok { color: #2a9d5c; font-weight: 600; }
  .fout { color: #c0392b; font-weight: 600; }
  #samenvatting2 { font-weight: 600; margin-top: 16px; }
</style></head>
<body>
  <h1>Voorschrijvers koppelen</h1>

  <div class="kaart">
    <h2>Stap 1 — Rapport genereren</h2>
    <p>Upload het originele bestand (tabblad 'Export'). Wijzigt niets — download het rapport,
    controleer/corrigeer de kolom 'Actie', en ga daarna naar stap 2.</p>
    <input type="file" id="bestand1" accept=".xlsx">
    <button onclick="genereerRapport()" id="btn1">Rapport genereren &amp; downloaden</button>
    <div id="status1"></div>
  </div>

  <div class="kaart">
    <h2>Stap 2 — Uitvoeren</h2>
    <p>Upload het (gecontroleerde) rapport terug. Dit koppelt daadwerkelijk de voorschrijvers aan de patiënten.</p>
    <input type="file" id="bestand2" accept=".xlsx">
    <button onclick="voerUit()" id="btn2">Koppelingen uitvoeren</button>
    <div id="resultaat2"></div>
  </div>

<script>
async function genereerRapport() {
  const input = document.getElementById('bestand1');
  ...
  formData.append('bestand', input.files[0]);
  try {
    const res = await fetch('/api/voorschrijvers-koppelen/rapport', { method: 'POST', body: formData });
    if (!res.ok) {
      const data = await res.json();
      status.innerHTML = '<p class="fout">Fout: ' + (data.fout || res.status) + '</p>';
      return;
    }
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'voorschrijvers_koppelen_rapport.xlsx'; a.click();
    URL.revokeObjectURL(url);
    status.innerHTML = '<p class="ok">✓ Rapport gedownload — controleer de kolom \\'Actie\\' voor je naar stap 2 gaat.</p>';
  } catch(e) {
    status.innerHTML = '<p class="fout">Onverwachte fout: ' + e.message + '</p>';
  } finally {
    btn.disabled = false; btn.textContent = 'Rapport genereren & downloaden';
  }
}

async function voerUit() {
  const input = document.getElementById('bestand2');
  const btn = document.getElementById('btn2');
  const resultaat = document.getElementById('resultaat2');
  if (!input.files[0]) { alert('Kies eerst een bestand.'); return; }
  btn.disabled = true; btn.textContent = 'Bezig…';
  resultaat.innerHTML = '';
  const formData = new FormData();
  formData.append('bestand', input.files[0]);
  try {
    const res = await fetch('/api/voorschrijvers-koppelen/uitvoeren', { method: 'POST', body: formData });
    const data = await res.json();
    if (data.fout) {
      resultaat.innerHTML = '<p class="fout">Fout: ' + data.fout + '</p>';
      return;
    }
    let html = `<div id="samenvatting2">${data.bijgewerkt} bijgewerkt, ${data.mislukt} mislukt (van ${data.totaal} bevestigde rijen, ${data.unieke_voorschrijvers} unieke voorschrijvers)</div>`;
    html += '<table><tr><th>Patient ID</th><th>Status</th><th>Details</th></tr>';
    data.resultaten.forEach(r => {
      const klasse = r.status === 'bijgewerkt' ? 'ok' : 'fout';
      const details = r.status === 'bijgewerkt' ? `voorschrijver: ${r.voorschrijver_naam} (id ${r.voorschrijver_id})` : r.fout;
      html += `<tr><td>${r.patient_id ?? '—'}</td><td class="${klasse}">${r.status}</td><td>${details}</td></tr>`;
    });
    html += '</table>';
    resultaat.innerHTML = html;
  } catch(e) {
    ...
    btn.disabled = false; btn.textContent = 'Koppelingen uitvoeren';
  }
}
</script>
</body></html>""")
...
@app.get("/api/rapport/bsn-corruptie")
async def rapport_bsn_corruptie():
    """
    Rapport (wijzigt niets) dat zoekt naar het patroon dat bij Berkel-Post/
    Robben is gevonden: hetzelfde BSN gekoppeld aan twee patiënten met een
    duidelijk ANDERE achternaam. Een BSN is uniek per persoon — als twee
    overduidelijk verschillende mensen (andere achternaam) hetzelfde BSN
    delen, is dat geen toeval maar een teken van datacorruptie (bijv. bij
    een migratie), in tegenstelling tot 'gewone' dubbele patiënten (waar een
    gedeeld BSN júist normaal is, want dezelfde persoon — zie
    /api/rapport/dubbele-patienten voor dat scenario).
    """
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    ...
    try:
        cur.execute("""
            SELECT p.id, p.email, p.voornaam, p.achternaam, p.geboortedatum, p.bsn,
                   p.telefoon, p.bijgewerkt_op,
                   (SELECT COUNT(*) FROM orders o WHERE o.patient_id = p.id) AS aantal_orders
            FROM patienten p
            WHERE p.bsn IS NOT NULL AND p.bsn <> ''
            ORDER BY p.bsn, p.id
        """)
        cols = ["id", "email", "voornaam", "achternaam", "geboortedatum", "bsn",
                "telefoon", "bijgewerkt_op", "aantal_orders"]
        patienten = [dict(zip(cols, r)) for r in cur.fetchall()]
    finally:
        cur.close()
        conn.close()

    per_bsn = defaultdict(list)
    for p in patienten:
        bsn_digits = "".join(ch for ch in (p["bsn"] or "") if ch.isdigit())
        if len(bsn_digits) == 9:
            per_bsn[bsn_digits].append(p)

    verdacht = []
    voor_info = []
    for bsn, leden in per_bsn.items():
        if len(leden) < 2:
            continue
        # Verschillende achternamen binnen dezelfde BSN-groep?
        achternamen_norm = set(_normaliseer_naam(l["achternaam"] or "") for l in leden)
        if len(achternamen_norm) > 1:
            verdacht.append((bsn, leden))
        else:
            voor_info.append((bsn, leden))

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Mogelijke BSN-corruptie"
    headers = ["BSN", "Patient ID", "Naam", "Geboortedatum", "E-mail", "Telefoon",
               "Aantal orders", "Laatst bijgewerkt"]
    ws.append(headers)
    for cel in ws[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="C0392B")
        cel.alignment = Alignment(horizontal="left")

    lichte_fill = PatternFill("solid", fgColor="FDECEA")
    for i, (bsn, leden) in enumerate(sorted(verdacht, key=lambda t: t[0])):
        for p in leden:
            ws.append([
                bsn, p["id"], f'{p["voornaam"] or ""} {p["achternaam"] or ""}'.strip(),
                str(p["geboortedatum"]) if p["geboortedatum"] else "",
                p["email"] or "", p["telefoon"] or "", p["aantal_orders"],
                str(p["bijgewerkt_op"])[:16] if p["bijgewerkt_op"] else "",
            ])
            if i % 2 == 0:
                for cel in ws[ws.max_row]:
                    cel.fill = lichte_fill
    for kolom, breedte in zip("ABCDEFGH", [11, 10, 24, 13, 26, 14, 12, 16]):
        ws.column_dimensions[kolom].width = breedte
    for row in ws.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ...
    # Tweede tabblad: de 'onschuldige' gevallen (zelfde BSN, zelfde achternaam
    # — gewone dubbele patiënten, geen corruptie) — ter referentie, niet de hoofdzaak.
    ws2 = wb.create_sheet("Ter info — zelfde achternaam")
    ws2.append(headers)
    for cel in ws2[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="5A9898")
    for bsn, leden in sorted(voor_info, key=lambda t: t[0]):
        for p in leden:
            ws2.append([
                bsn, p["id"], f'{p["voornaam"] or ""} {p["achternaam"] or ""}'.strip(),
                str(p["geboortedatum"]) if p["geboortedatum"] else "",
    ...
    for kolom, breedte in zip("ABCDEFGH", [11, 10, 24, 13, 26, 14, 12, 16]):
        ws2.column_dimensions[kolom].width = breedte
    ...
    ws3.append(["Totaal patiënten met BSN", len(patienten)])
    ws3.append(["Aantal BSN's gedeeld door 2+ patiënten", len(verdacht) + len(voor_info)])
    ws3.append(["  waarvan met VERSCHILLENDE achternaam (waarschijnlijk corruptie)", len(verdacht)])
    ws3.append(["  waarvan met DEZELFDE achternaam (waarschijnlijk gewone dubbeling)", len(voor_info)])
    ws3.append(["Betrokken patiëntrijen bij mogelijke corruptie", sum(len(l) for _, l in verdacht)])
    ...
    return Response(
        content=buf.getvalue(),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": "attachment; filename=bsn_corruptie_rapport.xlsx"},
    )


@app.get("/api/rapport/patienten-met-voorschrijver")
async def rapport_patienten_met_voorschrijver():
    """
    Rapport (wijzigt niets): overzicht van patiënten die al een gekoppelde
    voorschrijver hebben (naam en/of AGB-code), zodat je kunt zien wat er nu
    daadwerkelijk in de database staat — en, via het tweede tabblad, wat er
    nog ontbreekt.
    """
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    ...
    try:
        cur.execute("""
            SELECT p.id, p.voornaam, p.achternaam, p.email, p.geboortedatum,
                   p.voorschrijver_datum, p.bijgewerkt_op,
                   v.naam, v.agb_code,
                   (SELECT COUNT(*) FROM orders o WHERE o.patient_id = p.id) AS aantal_orders
            FROM patienten p
            LEFT JOIN voorschrijvers v ON v.id = p.voorschrijver_id
            ORDER BY p.voorschrijver_id IS NULL, v.naam, p.achternaam
        """)
        cols = ["id", "voornaam", "achternaam", "email", "geboortedatum", "voorschrijver_datum",
                "bijgewerkt_op", "voorschrijver_naam", "voorschrijver_agb_code", "aantal_orders"]
        alle = [dict(zip(cols, r)) for r in cur.fetchall()]
    finally:
        cur.close()
        conn.close()

    met = [p for p in alle if p["voorschrijver_naam"] or p["voorschrijver_agb_code"]]
    zonder = [p for p in alle if not (p["voorschrijver_naam"] or p["voorschrijver_agb_code"])]

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Met voorschrijver"
    headers = ["Patient ID", "Naam", "Geboortedatum", "E-mail", "Voorschrijver", "AGB-code",
               "Voorschrijver gekoppeld op", "Aantal orders", "Laatst bijgewerkt"]
    ws.append(headers)
    for cel in ws[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="5A9898")
    ...
    for p in met:
        ws.append([
            p["id"], f'{p["voornaam"] or ""} {p["achternaam"] or ""}'.strip(),
            str(p["geboortedatum"]) if p["geboortedatum"] else "", p["email"] or "",
            p["voorschrijver_naam"] or "", p["voorschrijver_agb_code"] or "",
            str(p["voorschrijver_datum"])[:16] if p["voorschrijver_datum"] else "",
            p["aantal_orders"], str(p["bijgewerkt_op"])[:16] if p["bijgewerkt_op"] else "",
        ])
    for kolom, breedte in zip("ABCDEFGHI", [10, 24, 13, 26, 22, 11, 18, 12, 16]):
        ws.column_dimensions[kolom].width = breedte
    for row in ws.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ...
    # Tweede tabblad: patiënten ZONDER voorschrijver — ter referentie
    ws2 = wb.create_sheet("Zonder voorschrijver")
    ws2.append(["Patient ID", "Naam", "Geboortedatum", "E-mail", "Aantal orders", "Laatst bijgewerkt"])
    for cel in ws2[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="9C9890")
        cel.alignment = Alignment(horizontal="left")
    for p in zonder:
        ws2.append([
            p["id"], f'{p["voornaam"] or ""} {p["achternaam"] or ""}'.strip(),
    ...
    for kolom, breedte in zip("ABCDEF", [10, 24, 13, 26, 12, 16]):
        ws2.column_dimensions[kolom].width = breedte
    for row in ws2.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ...
    # Samenvatting
    ws3 = wb.create_sheet("Samenvatting")
    unieke_voorschrijvers = len(set(p["voorschrijver_naam"] for p in met if p["voorschrijver_naam"]))
    zonder_agb = len([p for p in met if p["voorschrijver_naam"] and not p["voorschrijver_agb_code"]])
    rijen = [
        ("Totaal patiënten", len(alle)),
        ("Met gekoppelde voorschrijver", len(met)),
        ("Zonder gekoppelde voorschrijver", len(zonder)),
        ("Percentage met voorschrijver", f"{round(len(met) / len(alle) * 100, 1) if alle else 0}%"),
        ("Unieke voorschrijvers (op naam)", unieke_voorschrijvers),
        ("  waarvan met voorschrijvernaam maar zonder AGB-code", zonder_agb),
    ]
    for label, waarde in rijen:
        ws3.append([label, waarde])
    for row in ws3.iter_rows():
        row[0].font = Font(name="Arial", bold=True)
        row[1].font = Font(name="Arial")
    ...
    return Response(
        content=buf.getvalue(),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": "attachment; filename=patienten_met_voorschrijver.xlsx"},
    )


@app.get("/api/debug-tabel-structuur")
async def debug_tabel_structuur(tabel: str = "voorschrijvers"):
    """TIJDELIJK — toont de daadwerkelijke kolommen (naam, type, nullable) van een tabel."""
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    conn = _pg_conn()
    cur = conn.cursor()
    try:
        ...
        kolommen = [{"naam": r[0], "type": r[1], "nullable": r[2]} for r in cur.fetchall()]
    finally:
        cur.close()
        conn.close()
    return JSONResponse(content={"tabel": tabel, "kolommen": kolommen})


@app.post("/api/voorschrijvers-samenvoegen-handmatig")
async def voorschrijvers_samenvoegen_handmatig(request: Request):
    """
    Direct, gericht twee of meer bekende voorschrijver-ID's samenvoegen —
    voor als je de ID's al zeker weet (bijv. uit een rapport) en niet het
    hele bestand-upload-traject opnieuw wilt doorlopen. Body:
    {"hoofd_id": 555, "andere_ids": [166], "canonieke_naam": "R LIEBREGTS"}
    'canonieke_naam' is optioneel — zonder dat veld blijft de naam van de
    hoofdrij ongewijzigd.
    """
    body = await request.json()
    hoofd_id = body.get("hoofd_id")
    andere_ids = body.get("andere_ids") or []
    canonieke_naam = body.get("canonieke_naam")

    if not hoofd_id or not andere_ids:
        return JSONResponse(status_code=400, content={"fout": "hoofd_id en andere_ids (niet-leeg) zijn verplicht"})
    if hoofd_id in andere_ids:
        return JSONResponse(status_code=400, content={"fout": "hoofd_id mag niet ook in andere_ids staan"})

    conn = _pg_conn()
    cur = conn.cursor()
    try:
        cur.execute("SELECT id, naam, agb_code FROM voorschrijvers WHERE id = %s", (hoofd_id,))
        hoofd = cur.fetchone()
        if not hoofd:
            return JSONResponse(status_code=404, content={"fout": f"voorschrijver {hoofd_id} niet gevonden"})

        cur.execute("SELECT id, naam, agb_code FROM voorschrijvers WHERE id = ANY(%s)", (andere_ids,))
        andere = cur.fetchall()
        gevonden_ids = [r[0] for r in andere]
        ontbrekend = [i for i in andere_ids if i not in gevonden_ids]

        nieuwe_naam = canonieke_naam or hoofd[1]
        nieuwe_agb = hoofd[2] or next((r[2] for r in andere if r[2]), None)
        cur.execute("UPDATE orders SET voorschrijver_id = %s WHERE voorschrijver_id = ANY(%s)", (hoofd_id, gevonden_ids))
        orders_verplaatst = cur.rowcount
        cur.execute("UPDATE patienten SET voorschrijver_id = %s WHERE voorschrijver_id = ANY(%s)", (hoofd_id, gevonden_ids))
        verplaatst = cur.rowcount
        cur.execute("UPDATE voorschrijvers SET agb_code = NULL WHERE id = ANY(%s)", (gevonden_ids,))
        cur.execute("UPDATE voorschrijvers SET naam = %s, agb_code = COALESCE(%s, agb_code), bijgewerkt_op = NOW() WHERE id = %s",
                    (nieuwe_naam, nieuwe_agb, hoofd_id))
        cur.execute("DELETE FROM voorschrijvers WHERE id = ANY(%s)", (gevonden_ids,))
        conn.commit()

        return JSONResponse(content={
            "ok": True, "hoofd_id": hoofd_id, "naam": nieuwe_naam, "agb_code": nieuwe_agb,
            "verwijderde_ids": gevonden_ids, "niet_gevonden_ids": ontbrekend,
            "patienten_verplaatst": verplaatst, "orders_verplaatst": orders_verplaatst,
        })
    except Exception as e:
        conn.rollback()
        return JSONResponse(status_code=500, content={"ok": False, "fout": str(e)})
...
@app.post("/api/voorschrijvers-dedupliceren/uitvoeren")
async def voorschrijvers_dedupliceren_uitvoeren(bestand: UploadFile = File(...)):
    """
    Voert de op tabblad 'Zelfde achternaam' bevestigde voorschrijver-
    samenvoegingen door. Groepeert de rijen op kolom B (Canonieke naam) —
    rijen met dezelfde canonieke naam zijn dezelfde persoon en worden
    samengevoegd; rijen met een eigen, andere canonieke naam blijven apart
    (hooguit de spelling wordt gelijkgetrokken).

    Per canonieke-naam-groep:
      - Zoek voor elke naam-variant (kolom C) de bijbehorende rij in onze
        eigen voorschrijvers-tabel (exacte naam-match). Een variant die
        alleen uit het AIS-bestand komt (nooit bij ons aangemaakt) heeft
        vaak geen eigen rij — die wordt overgeslagen, er is dan niets om
        samen te voegen.
      - 2+ gevonden rijen -> samenvoegen: de rij met de meeste gekoppelde
        patiënten wordt de hoofdrij (naam bijgewerkt naar de canonieke
        spelling, AGB-code aangevuld als die nog ontbrak), de andere
        rij(en) se patiënten.voorschrijver_id wordt omgezet naar de
        hoofdrij, en de nu lege rij(en) worden verwijderd.
      - 0-1 gevonden rijen -> geen samenvoeging nodig, alleen de spelling
        bijwerken naar de canonieke naam (als die afwijkt).
    """
    import openpyxl
    from io import BytesIO
    ...
    if "Zelfde achternaam" not in wb.sheetnames:
        return JSONResponse(status_code=400, content={"fout": "Tabblad 'Zelfde achternaam' niet gevonden — is dit het juiste rapport?"})
    ws = wb["Zelfde achternaam"]

    from collections import defaultdict
    per_canoniek = defaultdict(list)
    for row in ws.iter_rows(min_row=2, values_only=True):
        if not row or not row[1] or not row[2]:
            continue
        canoniek, variant = str(row[1]).strip(), str(row[2]).strip()
        per_canoniek[canoniek].append(variant)

    resultaten = []
    for canoniek, varianten in per_canoniek.items():
        varianten = list(dict.fromkeys(varianten))  # dubbelen in het bestand zelf eruit, volgorde behouden
        conn = _pg_conn()
        cur = conn.cursor()
        try:
            gevonden = []
            ...
            for variant in varianten:
                cur.execute(
                    "SELECT id, naam, agb_code, praktijk, email, telefoon, "
                    "(SELECT COUNT(*) FROM patienten p WHERE p.voorschrijver_id = voorschrijvers.id) AS aantal_patienten "
                    "FROM voorschrijvers WHERE naam = %s",
                    (variant,),
                )
                rij = cur.fetchone()
                if not rij:
                    # Terugval: spaties/hoofdletters-ongevoelig — vangt kleine
                    # tekstverschillen op die de exacte match zou missen.
                    cur.execute(
                        "SELECT id, naam, agb_code, praktijk, email, telefoon, "
                        "(SELECT COUNT(*) FROM patienten p WHERE p.voorschrijver_id = voorschrijvers.id) AS aantal_patienten "
                        "FROM voorschrijvers WHERE LOWER(TRIM(naam)) = LOWER(TRIM(%s))",
                        (variant,),
                    )
                    rij = cur.fetchone()
                    if rij:
                        niet_exact.append({"variant": variant, "gevonden_als": rij[1]})
                if rij:
                    gevonden.append({
                        "id": rij[0], "naam": rij[1], "agb_code": rij[2], "praktijk": rij[3],
                        "email": rij[4], "telefoon": rij[5], "aantal_patienten": rij[6],
                    })
            # Dubbele database-rijen (zelfde id via twee varianten gevonden) eruit
            gevonden = list({v["id"]: v for v in gevonden}.values())

            if not gevonden:
                resultaten.append({"canoniek": canoniek, "varianten": varianten, "status": "overgeslagen",
                                    "reden": "geen van de varianten bestaat als voorschrijver-rij in onze database"})
                conn.rollback(); cur.close(); conn.close()
                continue

            if len(gevonden) == 1:
                hoofd = gevonden[0]
                if hoofd["naam"] != canoniek:
                    cur.execute("UPDATE voorschrijvers SET naam = %s, bijgewerkt_op = NOW() WHERE id = %s",
                                (canoniek, hoofd["id"]))
                    conn.commit()
                    resultaten.append({"canoniek": canoniek, "varianten": varianten, "status": "naam bijgewerkt",
                                        "hoofd_id": hoofd["id"], "niet_exact": niet_exact})
                else:
                    resultaten.append({"canoniek": canoniek, "varianten": varianten, "status": "ongewijzigd (al correct)",
                                        "hoofd_id": hoofd["id"], "niet_exact": niet_exact})
                cur.close(); conn.close()
                continue

            # 2+ gevonden -> samenvoegen. Hoofdrij = de meeste gekoppelde patiënten.
            gevonden.sort(key=lambda v: (-v["aantal_patienten"], v["id"]))
            hoofd = gevonden[0]
            andere = gevonden[1:]
            andere_ids = [v["id"] for v in andere]

            nieuwe_agb = hoofd["agb_code"] or next((v["agb_code"] for v in andere if v["agb_code"]), None)

            # Volgorde is belangrijk: eerst alle verwijzingen ÉN de AGB-code
            # van de rijen die verdwijnen opruimen, pas dán de hoofdrij
            # bijwerken en de oude rijen verwijderen — anders wijst
            # 'orders' nog naar een rij die niet meer bestaat (foreign key),
            # of botsen twee rijen even kort op dezelfde AGB-code (unique).
            cur.execute("UPDATE orders SET voorschrijver_id = %s WHERE voorschrijver_id = ANY(%s)",
                        (hoofd["id"], andere_ids))
            verplaatste_orders = cur.rowcount
            cur.execute("UPDATE patienten SET voorschrijver_id = %s WHERE voorschrijver_id = ANY(%s)",
                        (hoofd["id"], andere_ids))
            verplaatste_patienten = cur.rowcount
            cur.execute("UPDATE voorschrijvers SET agb_code = NULL WHERE id = ANY(%s)", (andere_ids,))
            cur.execute(
                "UPDATE voorschrijvers SET naam = %s, agb_code = COALESCE(%s, agb_code), bijgewerkt_op = NOW() WHERE id = %s",
                (canoniek, nieuwe_agb, hoofd["id"]),
            )
            cur.execute("DELETE FROM voorschrijvers WHERE id = ANY(%s)", (andere_ids,))
            conn.commit()

            resultaten.append({
                "canoniek": canoniek, "varianten": varianten, "status": "samengevoegd",
                "hoofd_id": hoofd["id"], "verwijderde_ids": andere_ids, "agb_code": nieuwe_agb,
                "patienten_verplaatst": verplaatste_patienten, "orders_verplaatst": verplaatste_orders,
                "niet_exact": niet_exact,
            })
        except Exception as e:
            conn.rollback()
            resultaten.append({"canoniek": canoniek, "varianten": varianten, "status": "mislukt", "fout": str(e)})
        finally:
            cur.close()
            conn.close()

    return JSONResponse(content={
        "totaal_groepen": len(resultaten),
        "samengevoegd": len([r for r in resultaten if r["status"] == "samengevoegd"]),
        "naam_bijgewerkt": len([r for r in resultaten if r["status"] == "naam bijgewerkt"]),
        "ongewijzigd": len([r for r in resultaten if r["status"] == "ongewijzigd (al correct)"]),
        "overgeslagen": len([r for r in resultaten if r["status"] == "overgeslagen"]),
        "mislukt": len([r for r in resultaten if r["status"] == "mislukt"]),
        "resultaten": resultaten,
    })
...
@app.get("/admin/voorschrijvers-dedupliceren", response_class=HTMLResponse)
async def admin_voorschrijvers_dedupliceren():
    """Uploadschermpje voor /api/voorschrijvers-dedupliceren/uitvoeren."""
    return HTMLResponse(content="""<!DOCTYPE html>
<html lang="nl"><head><meta charset="UTF-8">
<title>Voorschrijvers dedupliceren</title>
<style>
  body { font-family: 'DM Sans', Arial, sans-serif; max-width: 760px; margin: 40px auto; padding: 0 20px; color: #2c2c2c; }
  h1 { font-size: 22px; font-weight: 600; }
  p { color: #555; line-height: 1.5; }
  .kaart { background: #fff; border: 1px solid #ddd; border-radius: 10px; padding: 20px; margin-top: 20px; }
  input[type=file] { display: block; margin: 12px 0; }
...
<body>
  <h1>Voorschrijvers dedupliceren</h1>
  <p>Upload het gecontroleerde rapport (tabblad 'Zelfde achternaam' met ingevulde canonieke namen in kolom B).
  Dit voert de samenvoegingen direct door — er is geen aparte controlestap.</p>
  <div class="kaart">
    <input type="file" id="bestand" accept=".xlsx">
    <button onclick="voerUit()" id="btn">Samenvoegen uitvoeren</button>
    <div id="resultaat"></div>
  </div>
<script>
async function voerUit() {
  ...
  formData.append('bestand', input.files[0]);
  try {
    const res = await fetch('/api/voorschrijvers-dedupliceren/uitvoeren', { method: 'POST', body: formData });
    const data = await res.json();
    if (data.fout) {
      resultaat.innerHTML = '<p class="fout">Fout: ' + data.fout + '</p>';
      return;
    }
    let html = `<div id="samenvatting">${data.samengevoegd} samengevoegd, ${data.naam_bijgewerkt} naam bijgewerkt, ${data.ongewijzigd} al correct, ${data.overgeslagen} overgeslagen, ${data.mislukt} mislukt (${data.totaal_groepen} canonieke namen totaal)</div>`;
    html += '<table><tr><th>Canonieke naam</th><th>Status</th><th>Details</th></tr>';
    data.resultaten.forEach(r => {
      let klasse = r.status === 'samengevoegd' || r.status === 'naam bijgewerkt' ? 'ok' : (r.status.includes('overgeslagen') || r.status.includes('ongewijzigd') ? 'skip' : 'fout');
      let details = '';
      if (r.status === 'samengevoegd') {
        details = `hoofd-id: ${r.hoofd_id}, verwijderd: ${r.verwijderde_ids.join(', ')}, AGB: ${r.agb_code || '—'}, patiënten verplaatst: ${r.patienten_verplaatst}`;
      } else if (r.status === 'naam bijgewerkt') {
        details = `hoofd-id: ${r.hoofd_id}`;
      } else if (r.status === 'mislukt') {
        details = r.fout;
      } else {
        details = r.reden || '';
      }
      html += `<tr><td>${r.canoniek}</td><td class="${klasse}">${r.status}</td><td>${details}</td></tr>`;
    });
    html += '</table>';
    resultaat.innerHTML = html;
  } catch(e) {
...
@app.get("/api/rapport/voorschrijvers-dedupliceren")
async def rapport_voorschrijvers_dedupliceren():
    """
    Rapport (wijzigt niets): zoekt in de ACTUELE voorschrijvers-tabel naar
    mogelijke dubbelingen — dezelfde kern-achternaam (na verwijderen van
    punten/streepjes/tussenvoegsels) met andere voorletters, plus een apart
    tabblad met alleen-gelijkende (mogelijke tikfout) achternamen.

    Zelfde aanpak/detectielogica als het eerdere handmatige rapport
    (patienten_met_voorschrijver.xlsx + AIS-export samengevoegd), nu als
    vaste route rechtstreeks op de database — dus herhaalbaar zonder
    bestanden te hoeven exporteren/uploaden, bijvoorbeeld na een
    samenvoeg-ronde om te zien wat er nog overblijft.

    Vul kolom B ('Canonieke naam') in en upload het resultaat naar
    /api/voorschrijvers-dedupliceren/uitvoeren om te bevestigen — exact
    dezelfde vervolgstap als eerder.
    """
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    ...
    def _splits_naam(naam):
        delen = naam.split(None, 1)
        return (delen[0], delen[1]) if len(delen) == 2 else ("", naam)

    def _kern_achternaam(achternaam):
        n = _normaliseer(achternaam)
        n = re.sub(r'[.\-]', ' ', n)
        n = re.sub(r'\b(van der|van den|van de|van|de|den|der|het|\'t)\b', ' ', n)
        return re.sub(r'\s+', ' ', n).strip()

    def _kern_voorletters(voorletters):
        return re.sub(r'[^a-z]', '', _normaliseer(voorletters))

    def _voorletters_verenigbaar(a, b):
        ka, kb = _kern_voorletters(a), _kern_voorletters(b)
        if not ka or not kb:
            return True
    ...
    try:
        cur.execute("""
            SELECT v.id, v.naam, v.agb_code,
                   (SELECT COUNT(*) FROM patienten p WHERE p.voorschrijver_id = v.id) AS aantal_patienten
            FROM voorschrijvers v
            ORDER BY v.naam
        """)
        voorschrijvers = [{"id": r[0], "naam": r[1], "agb_code": r[2], "aantal_patienten": r[3]} for r in cur.fetchall()]
    finally:
        cur.close()
        conn.close()

    per_naam = {v["naam"]: v for v in voorschrijvers}  # naam is uniek (zie _zoek_of_maak_voorschrijver)
    unieke_namen = list(per_naam.keys())

    per_kern = defaultdict(list)
    for naam in unieke_namen:
        _, achternaam = _splits_naam(naam)
        per_kern[_kern_achternaam(achternaam)].append(naam)
    groep1 = {k: v for k, v in per_kern.items() if len(v) > 1}

    gezien = set(n for namen in groep1.values() for n in namen)
    data = [(naam, _splits_naam(naam)[1]) for naam in unieke_namen]
    paren = []
    for i in range(len(data)):
        for j in range(i + 1, len(data)):
            naam_a, ach_a = data[i]
            naam_b, ach_b = data[j]
            if naam_a in gezien and naam_b in gezien:
                continue
            ka, kb = _kern_achternaam(ach_a), _kern_achternaam(ach_b)
            if not ka or not kb or ka == kb:
                continue
            score = fuzz.ratio(ka, kb)
            if score >= 75:
                paren.append((score, naam_a, naam_b))
    paren.sort(key=lambda t: -t[0])

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Zelfde achternaam"
    headers = ["Groep", "Canonieke naam (invullen)", "Naam-variant", "Voorletters verenigbaar?",
               "AGB-code bekend", "Aantal patiënten", "Voorschrijver-ID"]
    ws.append(headers)
    for cel in ws[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="5A9898")
    ...
    groep_nr = 0
    lichte_fill = PatternFill("solid", fgColor="E8F4F4")
    for kern, namen in sorted(groep1.items(), key=lambda kv: -sum(per_naam[n]["aantal_patienten"] for n in kv[1])):
        groep_nr += 1
        namen_gesorteerd = sorted(namen, key=lambda n: -per_naam[n]["aantal_patienten"])
        hoofd = namen_gesorteerd[0]
        alle_verenigbaar = all(_voorletters_verenigbaar(hoofd, n) for n in namen_gesorteerd[1:])
        for i, naam in enumerate(namen_gesorteerd):
            v = per_naam[naam]
            ws.append([
                groep_nr,
                hoofd if (alle_verenigbaar and i == 0) else "",
                naam,
                "Ja" if _voorletters_verenigbaar(hoofd, naam) else "Twijfel — controleer",
                v["agb_code"] or "",
                v["aantal_patienten"],
                v["id"],
            ])
            if groep_nr % 2 == 0:
                for cel in ws[ws.max_row]:
                    cel.fill = lichte_fill
    for kolom, breedte in zip("ABCDEFG", [7, 26, 22, 20, 16, 14, 12]):
        ws.column_dimensions[kolom].width = breedte
    for row in ws.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ...
    ws2 = wb.create_sheet("Gelijkende achternaam")
    ws2.append(["Score", "Naam A", "Naam B", "Zelfde persoon (Ja/Nee)", "AGB A", "AGB B"])
    for cel in ws2[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="854F0B")
        cel.alignment = Alignment(horizontal="left")
    for score, a, b in paren:
        ws2.append([round(score), a, b, "", per_naam[a]["agb_code"] or "", per_naam[b]["agb_code"] or ""])
    for kolom, breedte in zip("ABCDEF", [8, 24, 24, 22, 14, 14]):
        ws2.column_dimensions[kolom].width = breedte
    for row in ws2.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ...
    ws3 = wb.create_sheet("Samenvatting")
    ws3.append(["Totaal voorschrijvers", len(voorschrijvers)])
    ws3.append(["Groepen met zelfde kern-achternaam", len(groep1)])
    ws3.append(["  betrokken namen", sum(len(v) for v in groep1.values())])
    ws3.append(["Mogelijke tikfout-paren (tabblad 2)", len(paren)])
    for row in ws3.iter_rows():
        row[0].font = Font(name="Arial", bold=True)
        row[1].font = Font(name="Arial")
    ...
    return Response(
        content=buf.getvalue(),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": "attachment; filename=voorschrijvers_dedupliceren.xlsx"},
    )


@app.get("/api/debug-zoek-voorschrijver")
async def debug_zoek_voorschrijver(fragment: str):
    """TIJDELIJK — zoekt voorschrijvers waarvan de naam 'fragment' bevat, en
    toont de RUWE repr() van de naam (zodat onzichtbare tekens, zoals een
    speciale spatie, zichtbaar worden in plaats van gewoon een spatie te lijken)."""
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    conn = _pg_conn()
    cur = conn.cursor()
    try:
        cur.execute(
            "SELECT id, naam, agb_code, "
            "(SELECT COUNT(*) FROM patienten p WHERE p.voorschrijver_id = voorschrijvers.id) AS aantal_patienten "
            "FROM voorschrijvers WHERE naam ILIKE %s ORDER BY naam",
            (f"%{fragment}%",),
        )
        resultaten = [
            {"id": r[0], "naam_repr": repr(r[1]), "lengte": len(r[1]), "agb_code": r[2], "aantal_patienten": r[3]}
            for r in cur.fetchall()
        ]
    finally:
        cur.close()
        conn.close()
    return JSONResponse(content={"fragment": fragment, "aantal": len(resultaten), "resultaten": resultaten})


@app.get("/api/statistieken/voorschrijvers")
async def statistieken_voorschrijvers():
    """Telt hoeveel patiënten er zijn en hoeveel daarvan een gekoppelde voorschrijver hebben."""
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    conn = _pg_conn()
    cur = conn.cursor()
    try:
        cur.execute("SELECT COUNT(*) FROM patienten")
        totaal = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM patienten WHERE voorschrijver_id IS NOT NULL")
        met_voorschrijver = cur.fetchone()[0]
    finally:
        cur.close()
        conn.close()
    return JSONResponse(content={
        "totaal_patienten": totaal,
        "met_voorschrijver": met_voorschrijver,
        "zonder_voorschrijver": totaal - met_voorschrijver,
        "percentage_met_voorschrijver": round(met_voorschrijver / totaal * 100, 1) if totaal else 0,
    })


@app.get("/api/rapport/dubbele-patienten")
async def rapport_dubbele_patienten():
    """
    Rapport (puur ter inzage — wijzigt niets) van patiënten die vermoedelijk
    dezelfde persoon zijn onder verschillende e-mailadressen.

    Oorzaak: _zoek_of_maak_patient() hierboven matcht uitsluitend op exact
    e-mailadres. Een patiënt zonder eigen e-mailadres krijgt bij elke nieuwe
    order een net iets ander (zelf ingevuld) adres en dus telkens een nieuwe
    rij in 'patienten', ook al bestaat er al een dossier met dezelfde BSN of
    naam+geboortedatum.

    Groepeert op:
      - exact BSN (sterkste signaal, als 9 cijfers aanwezig), of anders
      - geboortedatum + genormaliseerde achternaam (dezelfde normalisatie als
        elders in de patiëntherkenning: _normaliseer_naam)
    Groepen van 3+ varianten die via een keten verbonden zijn (bijv. A+B delen
    een BSN, B+C delen alleen naam+geboortedatum) komen in dezelfde groep.
    Alleen groepen met 2 of meer patiënten worden opgenomen — dit rapport
    verandert niets aan de database, dat is een aparte, latere stap.
    """
    if not _DATABASE_URL:
        return JSONResponse(status_code=500, content={"fout": "Geen database verbinding"})
    ...
    try:
        cur.execute("""
            SELECT p.id, p.email, p.voornaam, p.achternaam, p.geboortedatum, p.bsn,
                   p.telefoon, p.adres, p.postcode, p.plaats, p.aangemaakt_op,
                   (SELECT COUNT(*) FROM orders o WHERE o.patient_id = p.id) AS aantal_orders
            FROM patienten p
            ORDER BY p.id
        """)
        cols = ["id", "email", "voornaam", "achternaam", "geboortedatum", "bsn", "telefoon",
                "adres", "postcode", "plaats", "aangemaakt_op", "aantal_orders"]
        patienten = [dict(zip(cols, r)) for r in cur.fetchall()]
    finally:
        cur.close()
        conn.close()

    # Union-find: koppelt patiënten die via BSN ÓF naam+geboortedatum met
    # elkaar verbonden zijn, ook via een keten van meerdere varianten.
    parent = {p["id"]: p["id"] for p in patienten}
    ...
    bsn_groepen, naam_groepen = {}, {}
    for p in patienten:
        bsn_digits = "".join(ch for ch in (p["bsn"] or "") if ch.isdigit())
        if len(bsn_digits) == 9:
            bsn_groepen.setdefault(bsn_digits, []).append(p["id"])
        if p["geboortedatum"] and p["achternaam"]:
            sleutel = (str(p["geboortedatum"]), _normaliseer_naam(p["achternaam"]))
            naam_groepen.setdefault(sleutel, []).append(p["id"])

    redenen = {}
    for ids in bsn_groepen.values():
        if len(ids) >= 2:
            for pid in ids:
                _voeg_samen(pid, ids[0])
                redenen.setdefault(pid, set()).add("BSN")
    for ids in naam_groepen.values():
        if len(ids) >= 2:
            for pid in ids:
                _voeg_samen(pid, ids[0])
                redenen.setdefault(pid, set()).add("Naam + geboortedatum")

    groepen = {}
    for p in patienten:
        groepen.setdefault(_vind(p["id"]), []).append(p)
    groepen = {root: leden for root, leden in groepen.items() if len(leden) >= 2}

    # --- Excel opbouwen ---
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Mogelijke dubbelingen"

    headers = ["Groep", "Patient ID", "Voornaam", "Achternaam", "Geboortedatum", "BSN",
               "E-mail", "Telefoon", "Adres", "Postcode", "Plaats", "Aantal orders",
               "Aangemaakt op", "Match-reden"]
    ws.append(headers)
    for cel in ws[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="5A9898")
    ...
    lichte_fill = PatternFill("solid", fgColor="E8F4F4")
    groep_nr = 0
    for root, leden in sorted(groepen.items(), key=lambda kv: -len(kv[1])):
        groep_nr += 1
        for p in sorted(leden, key=lambda p: -p["aantal_orders"]):
            reden = " + ".join(sorted(redenen.get(p["id"], []))) or "—"
            ws.append([
                groep_nr, p["id"], p["voornaam"] or "", p["achternaam"] or "",
                str(p["geboortedatum"]) if p["geboortedatum"] else "",
                p["bsn"] or "", p["email"] or "", p["telefoon"] or "",
                p["adres"] or "", p["postcode"] or "", p["plaats"] or "",
                p["aantal_orders"], str(p["aangemaakt_op"])[:16] if p["aangemaakt_op"] else "",
                reden,
            ])
            if groep_nr % 2 == 0:
                for cel in ws[ws.max_row]:
    ...
    for kolom, breedte in zip("ABCDEFGHIJKLMN", [7, 10, 14, 16, 13, 11, 26, 13, 22, 10, 14, 12, 15, 22]):
        ws.column_dimensions[kolom].width = breedte
    for row in ws.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ...
    # Tweede tabblad: korte samenvatting, zodat meteen duidelijk is hoe groot
    # het probleem is zonder de hele lijst door te hoeven scrollen.
    placeholder_domeinen = ("@farmamed.nl", "@apotheekwoerden.nl")
    placeholder_patienten = [
        p for p in patienten
        if not (p["email"] or "").strip()
        or (p["email"] or "").strip().lower().endswith(placeholder_domeinen)
    ]

    ws2 = wb.create_sheet("Samenvatting")
    totaal_dubbel_rijen = sum(len(l) for l in groepen.values())
    samen_rijen = [
        ("Totaal patiënten in database", len(patienten)),
        ("Aantal gevonden groepen", len(groepen)),
        ("Totaal betrokken patiëntrijen", totaal_dubbel_rijen),
        ("Naar schatting samen te voegen (rijen − groepen)", totaal_dubbel_rijen - len(groepen)),
        ("Patiënten zonder eigen e-mailadres (leeg of @farmamed.nl/@apotheekwoerden.nl)", len(placeholder_patienten)),
    ]
    for label, waarde in samen_rijen:
        ws2.append([label, waarde])
    for cel in ws2["A"]:
        cel.font = Font(name="Arial", bold=True)
    for row in ws2.iter_rows():
        for cel in row:
            if cel.column_letter != "A":
                cel.font = Font(name="Arial")
    ws2.column_dimensions["A"].width = 42
    ws2.column_dimensions["B"].width = 12

    # Derde tabblad: patiënten zonder eigen e-mailadres (leeg, of een eigen
    # @farmamed.nl/@apotheekwoerden.nl-placeholder) — dit zijn precies de
    # rijen waar bij een volgende order een nieuwe, net-iets-andere placeholder
    # ontstaat en dus een verkapte dubbeling van een ander dossier kan zijn.
    ws3 = wb.create_sheet("Placeholder e-mails")
    ws3.append(headers[1:])  # zelfde kolommen als hoofdtabblad, zonder 'Groep'
    for cel in ws3[1]:
        cel.font = Font(name="Arial", bold=True, color="FFFFFF")
        cel.fill = PatternFill("solid", fgColor="5A9898")
        cel.alignment = Alignment(horizontal="left")

    # (placeholder_patienten is hierboven al berekend, voor de samenvatting)
    for p in sorted(placeholder_patienten, key=lambda p: (p["achternaam"] or "").lower()):
        reden = " + ".join(sorted(redenen.get(p["id"], []))) or "—"
        ws3.append([
            p["id"], p["voornaam"] or "", p["achternaam"] or "",
            str(p["geboortedatum"]) if p["geboortedatum"] else "",
            p["bsn"] or "", p["email"] or "(leeg)", p["telefoon"] or "",
            p["adres"] or "", p["postcode"] or "", p["plaats"] or "",
            p["aantal_orders"], str(p["aangemaakt_op"])[:16] if p["aangemaakt_op"] else "",
            reden,
        ])
    for kolom, breedte in zip("ABCDEFGHIJKLM", [10, 14, 16, 13, 11, 26, 13, 22, 10, 14, 12, 15, 22]):
        ws3.column_dimensions[kolom].width = breedte
    for row in ws3.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ws3.freeze_panes = "A2"

    # Vierde tabblad: voor elke placeholder-e-mail-patiënt, de best passende
    # kandidaat ONDER de patiënten mét een eigen e-mailadres — op BSN,
    # geboortedatum, naam (fuzzy) en adres (alleen als extra ondersteunend
    # signaal, nooit als enige reden: een adres wordt vaak door een heel
    # gezin gedeeld). Zelfde rapidfuzz-bibliotheek en aanpak als de bestaande
    # patiëntherkenning in _zoek_patient_kandidaten, nu toegepast tussen twee
    # database-rijen in plaats van tussen een e-mail/recept en de database.
    from rapidfuzz import fuzz

    _placeholder_ids = {p["id"] for p in placeholder_patienten}
    echte_email_patienten = [p for p in patienten if p["id"] not in _placeholder_ids]

    def _match_score(p, q):
        score = 0
        redenen = []

        p_bsn = "".join(ch for ch in (p["bsn"] or "") if ch.isdigit())
        q_bsn = "".join(ch for ch in (q["bsn"] or "") if ch.isdigit())
        if len(p_bsn) == 9 and p_bsn == q_bsn:
            score = max(score, 99)
            redenen.append("BSN")

        p_gbd = _normaliseer_geboortedatum(str(p["geboortedatum"]) if p["geboortedatum"] else "")
        q_gbd = _normaliseer_geboortedatum(str(q["geboortedatum"]) if q["geboortedatum"] else "")
        if p_gbd and p_gbd == q_gbd:
            score = max(score, 93)
            redenen.append("Geboortedatum")

        p_naam = _normaliseer_naam(f'{p["voornaam"] or ""} {p["achternaam"] or ""}'.strip())
        q_naam = _normaliseer_naam(f'{q["voornaam"] or ""} {q["achternaam"] or ""}'.strip())
        if p_naam and q_naam:
            naam_score = max(fuzz.token_set_ratio(p_naam, q_naam), fuzz.token_sort_ratio(p_naam, q_naam))
            if naam_score >= 85:
                score = max(score, naam_score)
                redenen.append(f"Naam ({round(naam_score)}%)")

        p_adres = _normaliseer_naam(f'{p["adres"] or ""} {p["postcode"] or ""}'.strip())
        q_adres = _normaliseer_naam(f'{q["adres"] or ""} {q["postcode"] or ""}'.strip())
        if p_adres and q_adres and score > 0:
            adres_score = fuzz.token_set_ratio(p_adres, q_adres)
            if adres_score >= 85:
                score = min(99, score + 5)
                redenen.append(f"Adres ({round(adres_score)}%, ondersteunend)")

        return score, redenen

    ws4 = wb.create_sheet("Placeholder → kandidaat")
    ws4_headers = [
        "Score", "Match-redenen",
        "Placeholder ID", "Placeholder naam", "Placeholder geboortedatum", "Placeholder BSN",
        "Placeholder adres", "Placeholder orders",
        "Kandidaat ID", "Kandidaat naam", "Kandidaat e-mail", "Kandidaat geboortedatum",
        "Kandidaat BSN", "Kandidaat adres", "Kandidaat orders",
    ]
    ...
    matches_gevonden = []
    for p in placeholder_patienten:
        beste, beste_score, beste_redenen = None, 0, []
        for q in echte_email_patienten:
            score, redenen = _match_score(p, q)
            if score > beste_score:
                beste, beste_score, beste_redenen = q, score, redenen
        if beste and beste_score >= 60:
            matches_gevonden.append((beste_score, beste_redenen, p, beste))

    for score, redenen, p, q in sorted(matches_gevonden, key=lambda t: -t[0]):
        ws4.append([
            score, " + ".join(redenen),
            p["id"], f'{p["voornaam"] or ""} {p["achternaam"] or ""}'.strip(),
            str(p["geboortedatum"]) if p["geboortedatum"] else "", p["bsn"] or "",
            f'{p["adres"] or ""} {p["postcode"] or ""}'.strip(), p["aantal_orders"],
            q["id"], f'{q["voornaam"] or ""} {q["achternaam"] or ""}'.strip(), q["email"] or "",
            str(q["geboortedatum"]) if q["geboortedatum"] else "", q["bsn"] or "",
            f'{q["adres"] or ""} {q["postcode"] or ""}'.strip(), q["aantal_orders"],
        ])
    for kolom, breedte in zip(
        "ABCDEFGHIJKLMNO",
        [7, 28, 12, 20, 16, 11, 24, 8, 12, 20, 26, 16, 11, 24, 8]
    ):
        ws4.column_dimensions[kolom].width = breedte
    for row in ws4.iter_rows(min_row=2):
        for cel in row:
            cel.font = Font(name="Arial", size=10)
    ws4.freeze_panes = "A2"

    # Aantal in de samenvatting erbij (na berekening, dus los toegevoegd)
    ws2.append(["Placeholder-patiënten met een gevonden kandidaat (score ≥ 60)", len(matches_gevonden)])
    ws2.append(["Placeholder-patiënten zonder duidelijke kandidaat", len(placeholder_patienten) - len(matches_gevonden)])
    for row in ws2.iter_rows(min_row=ws2.max_row - 1):
        row[0].font = Font(name="Arial", bold=True)
        row[1].font = Font(name="Arial")
    ...
    return Response(
        content=buf.getvalue(),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": "attachment; filename=dubbele_patienten_rapport.xlsx"},
    )
...
async def download_recept(request: Request):
    ...
    try:
        ...
        # Het WERKELIJKE bestandstype bepalen aan de bytes zelf — de
        # content-type-header van de WordPress-plugin bleek onbetrouwbaar
        # (claimde 'application/pdf' ook bij een JPEG-inhoud).
        werkelijk_type = _detecteer_bestandstype(inhoud)

        if werkelijk_type in ("image/jpeg", "image/png", "image/gif"):
            # Recept kwam als foto binnen — omzetten naar een echte PDF,
            # zodat het downloadresultaat altijd consistent een PDF is,
            # ongeacht hoe het origineel is binnengekomen.
            try:
                inhoud = _afbeelding_naar_pdf(inhoud, werkelijk_type)
                ct = "application/pdf"
            except Exception as e:
                print(f"[download-recept] Omzetten naar PDF mislukt, geef origineel terug: {e}")
                ct = werkelijk_type
        else:
            ct = werkelijk_type if werkelijk_type != "application/octet-stream" else "application/pdf"
        ...
        juiste_extensie = _extensie_per_type.get(ct, ".pdf")
        basisnaam = bestandsnaam.rsplit(".", 1)[0] if "." in bestandsnaam else bestandsnaam
        bestandsnaam = basisnaam + juiste_extensie
...
async def download_bijlage(request: Request):
    ...
    try:
        ...
        # Zelfde aanpak als bij /api/download-recept: het WERKELIJKE
        # bestandstype uit de bytes zelf bepalen (het opgeslagen 'type' komt
        # oorspronkelijk wel uit de e-mail zelf en is doorgaans betrouwbaar,
        # maar we controleren het toch consistent op dezelfde manier), en een
        # foto omzetten naar een echte PDF zodat het resultaat altijd
        # consistent een PDF is.
        werkelijk_type = _detecteer_bestandstype(inhoud)
        if werkelijk_type in ("image/jpeg", "image/png", "image/gif"):
            try:
                inhoud = _afbeelding_naar_pdf(inhoud, werkelijk_type)
                ...
                print(f"[download-bijlage] Omzetten naar PDF mislukt, geef origineel terug: {e}")
                ct = werkelijk_type
        else:
            ct = werkelijk_type if werkelijk_type != "application/octet-stream" else bijlage.get("type", "application/pdf")

        # Extensie van de bestandsnaam corrigeren naar het WERKELIJKE
        # bestandstype — niet blind vertrouwen op wat de aanroeper als
        # bestandsnaam heeft meegestuurd.
        _extensie_per_type = {
            "application/pdf": ".pdf",
            "image/jpeg": ".jpg", "image/jpg": ".jpg",
            "image/png": ".png",
...
def _check_proefpakket_override(recept_data: dict) -> dict:
    """
    Sommige recepten (van specifieke voorschrijvers) noemen 'proefpakket' of
    'proefpakket topicale analgetica' in de opmerkingen — dit medicijn komt
    vaak niet voor in de standaard medicatielijst van de arts, waardoor de
    AI-uitlezing het niet als een concreet medicijn herkent. Een keiharde
    tekstcontrole is hier betrouwbaarder dan een AI-instructie proberen te
    laten onthouden: dit werkt altijd hetzelfde, ongeacht wat het model die
    dag toevallig doet met de instructie in de prompt.
    """
    _trefwoorden = ("proefpakket topicale analgetica", "proefpakket")
    _te_doorzoeken_velden = ("opmerkingen", "gebruiksaanwijzing", "medicijn")
    volledige_tekst = " ".join(str(recept_data.get(veld) or "") for veld in _te_doorzoeken_velden).lower()
    if any(trefwoord in volledige_tekst for trefwoord in _trefwoorden):
        recept_data["medicijn"] = "PROEFBEHANDELING PIJNSTILLENDE CREMES SAW KOPSKY"
    return recept_data


def _recept_ocr_prompt() -> str:
    """
    Prompt voor het uitlezen van een recept — gedeeld door /api/analyseer-recept
    (Balie-upload) en /api/lees-bijlage (e-mailbijlage). Was voorheen in beide
    functies apart uitgeschreven en op een gegeven moment uit elkaar gaan
    lopen (Balie miste de Afzender/Contactpersoon-herkenning en de
    dubbelzinnige-achternamen-instructie) — vandaar nu één centrale versie.
    """
    return """Analyseer dit recept en extraheer de velden als JSON.
Geef ALLEEN JSON terug, geen uitleg of markdown.

BELANGRIJK — structuur van een Nederlands recept (AIS/apotheeksysteem-printout):
- Bovenaan staat vrijwel altijd de ARTS/PRAKTIJK: naam, adres, telefoonnummer,
  AGB-code van de PRAKTIJK. Dit is NOOIT de patiënt, ook al staat het bovenaan.
- Onderaan (vaak na de medicatieregel(s), soms bij een handtekening) staat het
  PATIËNTBLOK: naam, straat + huisnummer, postcode + woonplaats, en vaak een
  geboortedatum en BSN. Dit patiëntblok kan een aanhef bevatten zoals "Dhr.",
  "Mevrouw", "Mevr.", gevolgd door een voorletter en achternaam (bijv.
  "Dhr. D. van Dijk" of "Mevrouw G. v. Leeuwen-Damen") — de aanhef zelf hoort
  niet in patient_naam, maar de voorletter(s) en achternaam wel.
- Volgorde binnen het patiëntblok: naam → straat + huisnummer → postcode + woonplaats.

BELANGRIJK — naam-notatie: soms staat de naam als "Achternaam, Voornamen"
(bijv. "H. Woensel, Hermana Eliza Theodora") — herken dit aan de komma, en
neem de VOLLEDIGE naam over zoals die er staat (voorletter(s) + achternaam
vóór de komma, volledige voornamen erna); wij ontleden dit zelf verder.

BELANGRIJK — verwar de patiënt niet met de arts/voorschrijver: een
voorschrijver-naam gaat vaak vooraf door "Arts:", een functietitel
("huisarts", "specialist"), of staat in het praktijk-briefhoofd bovenaan.
Gebruik die naam voor "voorschrijver", nooit voor "patient_naam". Let op:
sommige recepten noemen de arts niet onder het label "Arts:", maar onder
"Afzender" of "Contactpersoon" (vaak met "(huisarts)" erachter) — herken dit
net zo goed als voorschrijver.

BELANGRIJK — dubbelzinnige achternamen: sommige Nederlandse achternamen zijn
ook gewone woorden (bijv. "Stuitje", "Bakker", "Visser", "de Boer"). Behandel
zo'n woord in het patiëntblok gewoon als achternaam, niet als iets anders.
Twijfel je tussen een gok en null? Kies de gok — "patient_naam": null
blokkeert de hele verwerking, terwijl een verkeerde gok later gewoon
gecorrigeerd wordt doordat de apotheker uit een kandidatenlijst kiest.

{
  ...
  "opmerkingen": "eventuele losse opmerkingen/aantekeningen elders op het recept, of null",
  "patient_naam": "volledige naam van de PATIËNT (niet de arts/apotheek), of null",
  ...
  "voorschrijver": "naam arts (ook als die alleen onder Afzender/Contactpersoon staat)",
...
def _verwerk_recept_data(recept_data: dict) -> dict:
    """
    Nabewerking na het uitlezen van een recept — gedeeld door dezelfde twee
    endpoints als _recept_ocr_prompt() hierboven, om dezelfde reden.
    Proefpakket-detectie (keiharde tekstcontrole), BSN opschonen, en
    spaties uit het e-mailadres verwijderen (OCR leest een e-mailadres soms
    met een spatie erin, bijv. rond de @ of een regelafbreking).
    """
    recept_data = _check_proefpakket_override(recept_data)
    ...
    if recept_data.get("email"):
        recept_data["email"] = "".join(str(recept_data["email"]).split())
    return recept_data
...
async def lees_bijlage(request: Request):
    ...
    try:
        ...
        recept_data = _verwerk_recept_data(recept_data)
