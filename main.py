...
async def download_recept(request: Request):
    ...
    try:
        ...
        # Bepaal media type
        ct = resp.headers.get("content-type", "application/pdf").split(";")[0].strip().lower()

        # Extensie van de bestandsnaam corrigeren naar het WERKELIJKE
        # bestandstype — de aanroeper (browser) gokt de extensie vooraf
        # (meestal .pdf), maar hier weten we het pas écht zeker. Zonder
        # deze correctie krijgen JPEG-scans die als bijlage binnenkwamen een
        # .pdf-bestandsnaam terwijl de inhoud een JPEG blijft, waardoor geen
        # enkele PDF-lezer het bestand kan openen.
        _extensie_per_type = {
            "application/pdf": ".pdf",
...
async def download_bijlage(request: Request):
    ...
    try:
        ...
        # Zelfde correctie als bij /api/download-recept: de extensie van de
        # bestandsnaam moet overeenkomen met het WERKELIJKE, opgeslagen
        # content-type van deze bijlage — niet blind vertrouwen op wat de
        # aanroeper als bestandsnaam heeft meegestuurd.
        _extensie_per_type = {
            "application/pdf": ".pdf",
            "image/jpeg": ".jpg", "image/jpg": ".jpg",
            "image/png": ".png",
