...
async def get_patient(patient_id: int):
    ...
    try:
        ...
        # Haal apotheek-specifieke velden op uit PostgreSQL (smarthub, voorschrijver, recept)
        cur.execute("""
        ...
        for r in cur.fetchall():
            row_d = dict(zip(cols_o, r))
            if row_d["smarthub_verstuurd_op"]:
                row_d["smarthub_verstuurd_op"] = str(row_d["smarthub_verstuurd_op"])[:16]
            db_orders[str(row_d["wc_order_id"])] = row_d

        # Haal alle orders live op uit WooCommerce (status, bedrag, datum, medicijn)
        orders = []
