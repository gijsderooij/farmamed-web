...
async def verstuur_scenario_mail(order_id: int, request: Request):
    ...
    Haalt naam/e-mail/medicijn rechtstreeks uit de WooCommerce-order op, bouwt
    de mailtekst op (aanhef + 'u heeft besteld' + standaardtekst bij het
    scenario) en verstuurt die als klantnotitie — hetzelfde bewezen patroon
    als /api/order/{order_id}/verstuur-naar-klant en /vraag-recept-op
    hierboven: WooCommerce stuurt dan zelf de e-mail, geen aparte SMTP-code
    nodig. Wijzigt de orderstatus NIET — dat blijft stap 6 (afronden).
    """
    body = await request.json()
    ...
    try:
        resp_note = http_requests.post(
            f"{wc_url}/wp-json/wc/v3/orders/{order_id}/notes",
            headers=_wc_auth(wc_key, wc_secret),
            json={"note": mailtekst, "customer_note": True},
            timeout=15,
        )
        resp_note.raise_for_status()
    except Exception as e:
