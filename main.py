...
async def maak_order(request: Request):
    ...
    try:
        ...
        # Sla patiënt op in PostgreSQL database
        if _DATABASE_URL:
            try:
                billing_db = {
                    "email": data.get("email", ""),
                    "first_name": data.get("patient_naam", "").split()[0] if data.get("patient_naam", "").split() else "",
                    "last_name": " ".join(data.get("patient_naam", "").split()[1:]) if len(data.get("patient_naam", "").split()) > 1 else "",
                    "phone": data.get("telefoon", ""),
                    "address_1": data.get("straat", ""),
                    "postcode": data.get("postcode", ""),
                    "city": data.get("woonplaats", ""),
                }
                bsn_db = data.get("bsn", "")
                gbd_db = _nl_naar_amerikaans(data.get("geboortedatum", ""))
                patient_id = _zoek_of_maak_patient(billing_db, bsn_db)
                if patient_id and gbd_db:
                    conn_db = _pg_conn()
                    ...
                    cur_db.close()
                    conn_db.close()
                # Sla ook order op in DB
                if patient_id:
                    _sla_order_op(
                        wc_order_id=str(order["id"]),
                        patient_id=patient_id,
                        voorschrijver_id=None,
                        medicijn_id=_zoek_medicijn_id(data.get("medicijn", "")),
                        hoeveelheid=1,
                        recept_datum=data.get("recept_datum", ""),
                    )
            except Exception as e:
                print(f"[maak-order] DB fout: {e}")
